...
## Vulkan IR legalization for struct values carrying pointer fields:
## pass 1 lowers device-fn `var T` params to value params. Pass 2 flattens
## struct-with-pointer-field values into leaf scalars and ptr-leaf
## expressions. Pass 3 binds device-fn `ptr` params per call site. The
## shared taint, fn-table, reachability and ptr-index-fold helpers serve
## all three passes and the subgroup-guard module.
