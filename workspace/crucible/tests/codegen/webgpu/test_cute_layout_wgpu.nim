## CuTe Layout + GEMM tile — WebGPU (WGSL) backend
## Run with: nim c -r --hints:off --warnings:off \
##   workspace/crucible/tests/codegen/webgpu/test_cute_layout_wgpu.nim
...
when defined(webgpu):
  ...
  block:
    var ctx = initWgpu()
    defer: ctx.shutdown()
    let r = execWgpu(ctx, kernelWgsl, "cuteKernel", outputBytes=40, inputs=@[])
    ...
    echo "  OK — CuTe Layout + GEMM (WebGPU)"
else:
  echo "---\nTo run, install wgpu-native, then recompile with -d:webgpu"
