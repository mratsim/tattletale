## Metal generic device procs instantiated at multiple static-parameter
## values must emit one body per instantiation, since `signatureHash` erases
## static bindings and dedups all instantiations into one body.
##
## Run:
##   nim c -r --outdir:build/tests --nimcache:nimcache/tests workspace/crucible/tests/codegen/metal/test_metal_generic_inst_dedup.nim
...
proc runTest() =
  suite "Metal - generic device proc static-param instantiations":
