...
proc gdnMoeLayerWalk*[T; HaveNorm: static bool](
  ...
  ##
  ## Parameters, pointers naming their dtypes, the arena sections and weight
  ## layouts living in the module tables above:
  ##
  ## | parameter                               | shape, dtype, layout                                                                                                                                                       | producer                                                                                          | unit              |
  ## | --------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------- | ----------------- |
  ## | counters                                | (13, ) uint32 wave counters, one per stage, host-allocated, zeroed once by the host and self-reset by the launch's final threadgroup thereafter (the reset contract above) | the host zeroes once, this launch's stages add                                                    | stage counts      |
  ## | bfA                                     | the element-dtype arena of BfArenaLen elements, dense, the sections addressed by the s* offsets                                                                            | the engine binds it host-side, the launch's own stages produce                                    | T elements        |
  ## | f32A                                    | the f32 arena of F32ArenaLen elements, the decay values and the MoE fp32 partials                                                                                          | the gate-values and MoE stages produce                                                            | f32 elements      |
