...
## D is the static FWHT block (128). `bits`, `cb` and `mmode` are the static instantiation family
## (bits 1..8 × cb 0..2, MMODE 0/1).
##
## Grid and decode shape: grid x = N div 128, grid y = (M + 31) div
## 32, threadgroup = 32 lanes. K and N are 128-multiples. Rows ≥ the mode's x row limit
## are zero-filled on load. Rows ≥ M are skipped on store.
## Per 128-column K-block the sequence is fixed: predicated x load,
## suh pre-scale, tile-level FWHT-128, dequant GEMM into the fp32 accumulators,
...
proc exl3_gemv_fwd*(
    ...
    mmode: static int,
    D: static int) {.device.} =
  ## Computes the fused EXL3 decode-GEMV forward for one output block,
  ...
  ## - bits 1..8 × cb 0..2
  ## - MMODE 0 = the m=1 fast path with the compile-time x row limit 1
  ## - MMODE 1 = m ≤ 8 with the runtime M x row limit
  ## M is the runtime row count. Grid x = N div 128,
  ## grid y = (M + 31) div 32, threadgroup = 32 lanes.
