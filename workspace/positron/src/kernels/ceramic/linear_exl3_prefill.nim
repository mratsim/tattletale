...
# TODO - manual_linear_exl3_prefill_fp16 fails its Metal run (MSL "excess elements in scalar initializer")
#   fix owned by the runtime-dims genericity pass over this kernel


# ############################################################
#
#     Fused EXL3 prefill-GEMM forward (linear_exl3_prefill), Tile API port
#
# ############################################################
...
## cb0 is the production default codebook.
##
## Shapes: K and N must be 128-multiples. M is the runtime row count (16 = the prefill tile, 32 = a full tile).
## Rows ≥ M are zero-filled on load and skipped on store.
##
...
proc exl3_gemm_fwd*(
    ...
    cb: static int,
    D: static int) {.device.} =
  ## Full-family binding over the shared core (linear_exl3_core)
  ## - bits 1..8 × cb 0..2, the runtime M as the x row limit
  ...
  exl3_fwd_core(Out, x, trellis, suh, svh, M, K, N, bits, cb = cb, D = D)
