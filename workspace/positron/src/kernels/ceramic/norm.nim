...
# ############################################################
#
#     Fused residual-add RMSNorm (norm), Tile API port
#
# ############################################################
...
proc rms_norm_res_in_fwd*(
  ...
  let gdX = X.gd(shape = (-1, -1, -1, -1), stride = (8 * C, 0, 1, C))
  let gdR = R.gd(shape = (-1, -1, -1, -1), stride = (8 * C, 0, 1, C))
  let gdG = G.gd(shape = (-1, -1, -1, -1), stride = (0, 0, 1, 0))
  let gdOut = Out.gd(shape = (-1, -1, -1, -1), stride = (8 * C, 0, 1, C))
  ...
  for cb in 0'i32 ..< colBlocks:
    x_rtr.loadTileRows(gdX, (0, 0, cb, gid), M)
    res_rtr.loadTileRows(gdR, (0, 0, cb, gid), M)
  ...
  for cb in 0'i32 ..< colBlocks:
    ...
    gamma_rtr.loadTile(gdG, (0, 0, cb, 0))
    s_rtr.mul(s_rtr, gamma_rtr)
    gdOut.storeTileRows(s_rtr, (0, 0, cb, gid), M)
