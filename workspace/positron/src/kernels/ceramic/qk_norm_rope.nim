...
proc qk_norm_rope_fwd*(
  ...
  let gdX = X.gd(shape = (-1, -1, -1, -1), stride = (xTokenStride, 0, 128, 1))
  let gdG = G.gd(shape = (-1, -1, -1, -1), stride = (0, 0, 0, 1))
  let gdCos = Cos.gd(shape = (-1, -1, -1, -1), stride = (cosTokenStride * 64, 0, 64, 1))
  let gdSin = Sin.gd(shape = (-1, -1, -1, -1), stride = (cosTokenStride * 64, 0, 64, 1))
  let gdOut = Out.gd(shape = (-1, -1, -1, -1), stride = (8 * 128, 0, 128, 1))
  ...
  xReg.loadTile(gdX, (gidY, 0, xRowBlock, 0))
  sq.mul(xReg, xReg)
  ss.row_sum(sq)
  ...
  cosT.loadTile(gdCos, (gidY, 0, gidZ, 0))
  sinT.loadTile(gdSin, (gidY, 0, gidZ, 0))
  x1s.mul(x1, sinT)
  x2s.mul(x2, sinT)
  x2s.mul(x2s, -1.0'f32)
  t1.fmaMapFloat32(x1, cosT, x2s)
  t2.fmaMapFloat32(x2, cosT, x1s)
  gdOut.storeTile(t1, (tile, 0, 0, 0))
  gdOut.storeTile(t2, (tile, 0, 0, 1))
