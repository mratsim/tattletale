...
# ############################################################
#
#     Fused SiLU × Mul (ffn_silu), Tile API port
#
# ############################################################
...
proc silu_and_mul_fwd*(
  ...
  let gdOut = Out.gd(shape = (-1, -1, -1, -1), stride = (8 * N, 0, N, 1))
  var gate: rt_l(float16, 8, TileC)
  var up: rt_l(float16, 8, TileC)
  var outT: rt_l(float16, 8, TileC)
  gate.loadTileRows(gdX, (0, 0, ty, tx), M)
  up.loadTileRows(gdX, (0, 0, ty, tx + N div TileC), M)
  siluAndMulElem(outT, gate, up, actLimit)
  gdOut.storeTileRows(outT, (0, 0, ty, tx), M)
