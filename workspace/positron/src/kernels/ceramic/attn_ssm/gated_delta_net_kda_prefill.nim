...
proc kdaPrefillChunkScanAt*[El](
  ...
  ## | cumulogdecay          | (B·Hk, T, Dk) f32 per-channel cumulative log decay, one decay per KEY channel, the host's inclusive prefix of g                                                                              | host-computed                                                                                                        | natural-log decay exponent |
  ...
  while c0 < T:
    ...
    for t in 0 ..< cLen:
      ...
      for sIdx in 0 ..< t:
        ...
        uacc += kkVec.rowScalar() * uLoc[sIdx]
    ...
    for sIdx in 0 ..< cLen:
      ...
      pdEnd.map2(cumulogdecayEnd, cumulogdecayS, exp2((x - y) * Log2e))
      pdEnd.mul(pdEnd, ks32)
      let ws = uLoc[sIdx]
      s.addScaled(pdEnd, ws)
    c0 += int32(cLen)
