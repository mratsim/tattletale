...
##
## Composition tier's segment runners, the judge and the comparison walk.
## Each runner runs against the composed naive 13-stage reference.
##
## Every arena section is judged per element under the driver's two-term
## band model (`ceramic_mega_gdn_composition.nim`).
##
## - `runNormCheck`, the fused add+norm check, 32 samples over two lane geometries
## - `runComparison`, 3 seeded cases with the mega launch judged against the naive walk
## - `runChain`, 8 decode steps over carried state and ring
