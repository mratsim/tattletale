...
proc bitPattern*[T: uint16|float32](v: T): uint32 =
...
proc assertTailZero*[T: uint16|float32](buf: PageBuf[T], usedElems: int) =
  ...
  for i in usedElems ..< buf.elems:
    doAssert bitPattern(buf.hostPtr[i]) == 0'u32, "buffer written past its extent at element " & $i
...
proc assertReadUnchanged*[T: uint16|float32](buf: PageBuf[T], want: seq[T]) =
  ...
  for i in 0 ..< want.len:
    doAssert bitPattern(buf.hostPtr[i]) == bitPattern(want[i]), "kernel-read buffer modified at element " & $i
