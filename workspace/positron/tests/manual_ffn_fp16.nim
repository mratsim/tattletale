...
##   nim cpp -r --hints:off --warnings:off --outdir:build/wip --nimcache:nimcache/wip workspace/positron/tests/manual_ffn_fp16.nim
...
proc gatedMlpSiluReference(xf, wgf, wuf, wdf: seq[float32], M, K, NIntm, NOut: int): F.Tensor =
  ## Reference (transformers layers/ffn.nim), torch linear over the same
  ## fp16-rounded inputs, weights transposed to linear's (out, in) layout:
  ## out = down(silu(x·wg)·(x·wu)).
...
when isMainModule:
  runCppTest("gated_mlp_silu vs the torch reference (transformers layers/ffn.nim)", checkGatedMlpSilu)
