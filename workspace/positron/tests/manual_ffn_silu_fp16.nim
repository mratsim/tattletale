...
##   nim cpp -r --hints:off --warnings:off --outdir:build/wip --nimcache:nimcache/wip workspace/positron/tests/manual_ffn_silu_fp16.nim
...
proc siluAndMulReference(gs, us: seq[float32], M, N: int): F.Tensor =
  ## Reference (transformers layers/ffn.nim), act = silu(g)·u composed
  ## in fp32 over the same fp16-rounded inputs.
...
when isMainModule:
  runCppTest("silu_and_mul vs the torch reference (transformers layers/ffn.nim)", checkSiluAndMul)
