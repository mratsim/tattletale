...
##   nim c -r -d:opencl --hints:off --warnings:off \
##     workspace/positron/tests/codegen/opencl/test_opencl_add.nim
...
when defined(opencl):
  ...
  echo "---"
  echo "To run execution tests, recompile with -d:opencl"
  echo "Requires libOpenCL.so (e.g. from ocl-icd-opencl-dev package)"
