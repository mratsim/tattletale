...
type
  RopeElementWiseGatedAttention*[QKNorm] = ref object
    ## Grouped-query attention with RoPE and an element-wise output gate.
    ## Computes `o_proj(attn(q_rot, k_full, v_full) * sigmoid(gate))`.
    ## Created at model load. Weights and rotary are immutable after init.
    ##
    ##  - `q_proj` output width is `2 * num_qo_head * head_dim`.
    ##    Query and gate interleave per head, layout `[q | gate]`.
    ##  - The attention output is scaled by `sigmoid(gate)` before `o_proj`.
    ...
    when QKNorm isnot void:
      ...
      k_norm: QKNorm
# =============================================================================
# Data flow through RopeElementWiseGatedAttention
# =============================================================================
#
#   x (batch, seq, hidden)
#   │
#   ├─→ q_proj → reshape (batch, seq, num_qo, 2 * head_dim)
#   │       ├─→ narrow q → q_norm → applyRope → q_rot   (partial, rotary_dim)
#   │       └─→ narrow gate                             raw, pre-sigmoid
#   │
...
func initBase[QKNorm](
    _: type RopeElementWiseGatedAttention[QKNorm],
    layer_idx: int,
    name: string,
    q_proj, k_proj, v_proj, o_proj: Linear,
    num_qo_head, num_kv_head, head_dim: int,
    rotary: RotaryPositionEmbedding): RopeElementWiseGatedAttention[QKNorm] =
  ...
  RopeElementWiseGatedAttention[QKNorm](
    layer_idx: layer_idx,
    name: name,
    q_proj: q_proj,
    k_proj: k_proj,
...
func init*[QKNorm](
    ...
    num_qo_head, num_kv_head, head_dim: int,
    rotary: RotaryPositionEmbedding): RopeElementWiseGatedAttention[QKNorm] =
  ## Build the gated attention block with no qk-norms.
  initBase(RopeElementWiseGatedAttention[QKNorm], layer_idx, name,
...
func init*[QKNorm](
    ...
    num_qo_head, num_kv_head, head_dim: int,
    rotary: RotaryPositionEmbedding,
    q_norm, k_norm: QKNorm): RopeElementWiseGatedAttention[QKNorm] =
  ## Initialize RopeElementWiseGatedAttention.
  ##
  ## Args:
  ##   layer_idx: Layer index (0..num_layers-1)
  ##   name: Safetensor key prefix (e.g., "model.layers.23.self_attn")
  ##   q_proj, k_proj, v_proj, o_proj: Preinitialized projections. q_proj emits
  ##     `[q | gate]` per head, so its output width is `2 * num_qo_head * head_dim`.
  ...
  result = initBase(RopeElementWiseGatedAttention[QKNorm], layer_idx, name,
    q_proj, k_proj, v_proj, o_proj,
    num_qo_head, num_kv_head, head_dim, rotary)
  when QKNorm isnot void:
...
proc forward[QKNorm](
    self: RopeElementWiseGatedAttention[QKNorm],
    ctx: var InferenceContext,
    x: Tensor): Tensor =
  ## Forward pass for gated attention with paged KV cache.
  ##
  ## Args:
  ...
  ##
  ## Computes:
  ##   (q, gate) = self.q_proj(x) reshaped to `[q | gate]` per head
  ##   k = self.k_proj(x)
  ##   v = self.v_proj(x)
  ...
  ##   return self.o_proj(attn_out * sigmoid(gate))
  let batch = x.size(0)
  ...
  # Use separate Q, K, V projections (matching HF/Qwen3.5 full-attention layers)
  ...
  let qg_reshaped = q.reshape([batch, seq_len, num_qo, 2 * head_dim])
  let q_reshaped = qg_reshaped.narrow(3, 0, head_dim)
  let gate = qg_reshaped.narrow(3, head_dim, head_dim)
  ...
  # k_full/v_full are already (batch, seq, kv_heads, head_dim), the GQA input format.
...
template `()`*[QKNorm](layer: RopeElementWiseGatedAttention[QKNorm],
            ctx: var InferenceContext,
            x: Tensor): untyped =
  layer.forward(ctx, x)
