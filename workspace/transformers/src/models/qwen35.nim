...
proc parseQwen35Config(json: JsonNode): Qwen35Config =
  ...
  ## - `layer_types` kinds each of the 24 layers: alkGatedDeltaNet
  ##   (Gated DeltaNet) or alkAttention
...
type
  Qwen35DecoderLayer* = ref object
    ## One of the 24 hybrid decoder layers. Exactly one attention variant is
      ## non-nil per layer: `gdn` for the alkGatedDeltaNet layers, `attn`
    ## for the attention layers. The residual pattern matches the vendored
...
proc forward*(self: Qwen35DecoderLayer, ctx: var InferenceContext, hidden: Tensor): Tensor =
  ...
  ## - alkAttention: the gated full attention, KV pages in ctx
  let residual = hidden
...
proc forward*(self: Qwen35Model, ctx: var InferenceContext, input_ids: Tensor): Tensor =
  ## Text forward pass: embed → 24 hybrid layers → final RMSNorm → tied lm_head.
  ##
...
proc loadQwen35ModelRaw(modelPath: string, device = kCPU): Qwen35Model =
  ...
  ##
  ## Every layer loads its input/post_attention layernorms as 1 + w RMSNorms
  ## and its SwiGLU MLP. `config.layer_types[i]` picks the attention prefix,
  ## `linear_attn.*` or `self_attn.*`. The final norm carries the same
  ## 1 + w weight spelling, constant_bias 1, like every RMSNorm of this stack.
  ##
  ...
  for i in 0 ..< config.num_hidden_layers:
    ...
    let inputLN = RmsNorm.load(weightsSt, cfgJson, lp & "input_layernorm", device,
      constant_bias = 1.0)
    inc tensorRequests
    let postLN = RmsNorm.load(weightsSt, cfgJson, lp & "post_attention_layernorm", device,
      constant_bias = 1.0)
    inc tensorRequests
    ...
    layers[i] = Qwen35DecoderLayer.init(
      config.layer_types[i], inputLN, postLN, gdn, attn, mlp)
