...
## The attention-layer-kind catalogue and the HF `layer_types` key
## mapping shared by the checkpoint configuration readers; each model
## parses its own hybrid schedule beside its config reader.

type
  AttentionLayerKind* = enum
    ## The attention variants of a hybrid decoder stack. Each config
    ## maps into this catalogue at its parse bound
    ...
    alkMla           ## Multi-head latent attention (DeepSeek, GLM-4.7-Flash).
                     ...
                     ## no config key maps to it today

proc parseAttnFromHfTransformers*(raw: string, source: string): AttentionLayerKind =
  ## Map one raw `layer_types` entry of an HF transformers `config.json`
  ## to its catalogue kind, exact match. `source` names
