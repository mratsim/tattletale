...
proc parseIntList*(json: JsonNode, key: string): seq[int] =
  ...
  ## Any other top-level kind raises `ValueError` naming `key`, which keeps
  ...
  of JNull:
    ...
    raise newException(ValueError,
      "[ttt] " & key & ": expected a list of ints, an int or null, found " &
      $json.kind)
...
proc reqSharedExpertIntermediateSize*(textCfg: JsonNode): int =
  ## Read the shared-expert FFN width from a MoE text config.
  ## Absent, null, wrong-typed and non-positive values raise `ValueError`
  ## naming the key.
  textCfg{"shared_expert_intermediate_size"}.reqPosInt(
