...
proc main() =
  ...
  for device in devices:
    ...
    block layer2:
      var st = Safetensor.open(Layer2TraceDir / "layer02_trace.safetensor")
      ...
      let qn = RmsNorm.load(view, cfgJson, "model.layers.2.self_attn.q_norm", device = device)
      let kn = RmsNorm.load(view, cfgJson, "model.layers.2.self_attn.k_norm", device = device)
      let ln = RmsNorm.load(view, cfgJson, "model.layers.2.input_layernorm", device = device)
      let pln = RmsNorm.load(view, cfgJson, "model.layers.2.post_attention_layernorm", device = device)
