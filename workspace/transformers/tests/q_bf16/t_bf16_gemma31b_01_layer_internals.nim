...
## Layer-4/5 unit replay of the gemma-3-1b-it checkpoint, one assertion
...
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_gemma31b_01_layer_internals.nim
...
const
  ModelDir =
    currentSourcePath().parentDir() / ".." / "hf_models" / "gemma-3-1b-it"
  FixturePath =
    currentSourcePath().parentDir() / ".." / "fixtures" / "bf16-01-layer-internals" /
    "gemma-3-1b-it-layer-4-5" / "layer4-5-gemma-3-1b-it-00.safetensor"
...
when isMainModule:
  runCppTest("gemma3 1b layer-4/5 internals", main)
