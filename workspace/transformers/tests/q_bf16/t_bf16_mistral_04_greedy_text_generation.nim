...
## Greedy (temp=0) decoding of the Mistral-7B-v0.1 stack, the recorded
...
## - tie-eligible picks teacher-force the recorded token, the zero-margin
##   steps of the Hello chain take the recorded tie path
## - the Fox story prefill runs 5042 prompt tokens through the production
##   KV path, the only chain crossing the 4096 sliding window
##
## Requires the local model at tests/hf_models/Mistral-7B-v0.1 (gitignored).
##
## Run:
##   nim cpp -r --hints:off --warnings:off --passC:"-std=c++20" \
##     --outdir:build/tests --nimcache:nimcache/tests workspace/transformers/tests/q_bf16/t_bf16_mistral_04_greedy_text_generation.nim
...
const
  ...
  FixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" /
    "bf16-04-greedy-text-generation" / "Mistral-7B-v0.1"
  GreedyFixtureFiles = [
    "Hello_there_friend_32_steps.json.zst",
    "Big_blue_whales_eat_krill_32_steps.json.zst",
    "Fox_story_prefill_crosses_4096_window_32_steps.json.zst",
  ]
...
proc main(): bool =
  ...
  for f in GreedyFixtureFiles:
    ...
    # prefill past the 4096-token sliding window stays inside it.
...
when isMainModule:
  runCppTest("mistral greedy text generation", main)
