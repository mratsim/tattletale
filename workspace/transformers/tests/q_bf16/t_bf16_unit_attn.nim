...
## Per-op attention unit suite, shared across the dense ports. One recorded
## family per op: the Qwen3.5-0.8B layer-3 attention family, whose q_proj
## packs [q | gate] per head and which carries the recorded stats sidecar.
## The KV page paths run against the same recorded family, and the multi-page gather path is fixture-free (synthetic input).
##
## Build:
...
##     workspace/transformers/tests/q_bf16/t_bf16_unit_attn.nim
...
proc setupAttn(): RopeElementWiseGatedAttention[RmsNormOne] =
  ...
  let weights = SafetensorsCollection.open(ModelDir / "model.safetensors-00001-of-00001.safetensors")
...
proc normFixtureTest(caseNum: int, msg: string): bool =
  ## RmsNormOne in isolation, 0.00 vs vendored ground truth.
  var st = Safetensor.open(FixtureDir / ("norm-Qwen3.5-0.8B-0" & $caseNum & ".safetensor"))
...
proc main() =
  ...
  runCppTest "Gated full attention prefill (seq 8) vs fixture":
    proc(): bool =
      ...
      var st = Safetensor.open(FixtureDir / "attn-Qwen3.5-0.8B-00.safetensor")
      ...
      # Final layer output: block-level tolerance, then the signature
      # checks (match rate plus recorded stats against the stats file).
      let expected = st.getTensorOwned("output")
      ...
      assertMatchRate(output, expected, attnBudget,
        msg = "attention output match rate")
      let attnStatsFile = loadFingerprintStats(
        FixtureDir / "attn-Qwen3.5-0.8B-00.safetensor.stats")
      assertStats(output, attnStatsFile.statsTensor("output"), attnBudget,
        msg = "attention output stats")

      # Intermediates, recomputed through the layer's own components and compared
      # against the fixture's captured deterministic-op values.
      let seqLen = x.size(1)
  ...
  runCppTest "Gated full attention decode (single token, position 5) vs fixture":
    proc(): bool =
      let attn = setupAttn()
      var (ctx, pool) = newKVContext(numLayers = 24, kvHeads = 2, headDim = 256)
      var st = Safetensor.open(FixtureDir / "attn-Qwen3.5-0.8B-01.safetensor")
  ...
  # ──────────────────────────────────────────────────────────────────────────
  # KV cache stores unrotated values: the stored V equals the v_proj output
  # ──────────────────────────────────────────────────────────────────────────
  runCppTest "KV cache stores unrotated values (vs v_proj output)":
    proc(): bool =
      let attn = setupAttn()
      var (ctx, pool) = newKVContext(numLayers = 24, kvHeads = 2, headDim = 256)
      var st = Safetensor.open(FixtureDir / "attn-Qwen3.5-0.8B-00.safetensor")
      ...
      # Values carry no rotation: the stored V is the plain v_proj output.
      let vRaw = attn.v_proj.forward(x).reshape([1, 8, 2, 256]).permute([0, 2, 1, 3])
      assertAllClose(cachedV, vRaw, rtol = 0.0, abstol = 0.0,
        msg = "KV cache must store the unrotated values")
      true
  ...
  runCppTest "Multi-page write loop + gather (300 tokens = 1 full + 44 partial)":
    proc(): bool =
      let attn = setupAttn()
      var (ctx, pool) = newKVContext(numLayers = 24, kvHeads = 2, headDim = 256, maxSeq = 4096)
  ...
  echo "\nAll per-op attention tests passed!"
