...
##   workspace/transformers/tests/q_bf16/test_qwen35_02_attn.nim
...
proc setupAttn(): RopeGQAttention =
  ...
  ## layer with the fused `[q | gate]` projection.
...
proc normFixtureTest(caseNum: int, msg: string): bool =
  ...
  var (memFile, st) = openSafetensor(FixtureDir, "norm-Qwen3.5-0.8B-0" & $caseNum & ".safetensor")
  defer: close(memFile)
  let x = st.getTensorOwned("input")
  let expected = st.getTensorOwned("output")
  let w = st.getTensorOwned("weight")
  let norm = RmsNorm.init(w, eps = 1e-6, constant_bias = 1.0)
...
proc main() =
  ...
  runCppTest "Gated full attention prefill (seq 8) vs fixture":
    proc(): bool =
      ...
      var (memFile, st) = openSafetensor(FixtureDir, "attn-Qwen3.5-0.8B-00.safetensor")
      defer: close(memFile)
  ...
  runCppTest "Gated full attention decode (single token, position 5) vs fixture":
    proc(): bool =
      let attn = setupAttn()
      var (ctx, pool) = newKVContext(numLayers = 24, kvHeads = 2, headDim = 256)
      var (memFile, st) = openSafetensor(FixtureDir, "attn-Qwen3.5-0.8B-01.safetensor")
      defer: close(memFile)
