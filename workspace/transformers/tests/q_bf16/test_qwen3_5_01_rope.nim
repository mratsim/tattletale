...
##   workspace/transformers/tests/q_bf16/test_qwen3_5_01_rope.nim
...
proc main() =
  ...
  runCppTest "RoPE rotateHalf 64-wide - mathematical property":
    proc(): bool =
      var (memFile03, st) = openFixture("rope-Qwen3.5-0.8B-03.safetensor")
  ...
  runCppTest "RoPE partial applyRopeImpl prefill (batch=2, seq=8)":
    proc(): bool =
      var (memFile00, st) = openFixture("rope-Qwen3.5-0.8B-00.safetensor")
  ...
  runCppTest "RoPE partial applyRopeImpl decode (single token, position 5)":
    proc(): bool =
      var (memFile01, st) = openFixture("rope-Qwen3.5-0.8B-01.safetensor")
  ...
  runCppTest "RoPE partial applyRopeImpl scattered positions (index_select path)":
    proc(): bool =
      var (memFile02, st) = openFixture("rope-Qwen3.5-0.8B-02.safetensor")
  ...
  runCppTest "Qwen3.5 partial rotary cache (rotary_dim 64) vs HF cos/sin":
    proc(): bool =
      ...
      var (memFile02b, st2) = openFixture("rope-Qwen3.5-0.8B-02.safetensor")
