...
const
  ...
  FixtureDirLongResidual = currentSourcePath().parentDir() / ".." / "fixtures" / "long-residual-3-block" / "Qwen3-0.6B"
...
proc main() =
  ...
  runCppTest "RoPE rotateHalf — mathematical property":
    proc(): bool =
      var st = Safetensor.open(FixtureDir_Layers / "rope-Qwen3-0.6B-02.safetensor")
  ...
  runCppTest "RoPE applyRopeImpl (batch=2, seq=8, GQA) — mathematical property":
    proc(): bool =
      var st = Safetensor.open(FixtureDir_Layers / "rope-Qwen3-0.6B-00.safetensor")
  ...
  runCppTest "RoPE applyRopeImpl (batch=1, seq=1) — mathematical property":
    proc(): bool =
      var st = Safetensor.open(FixtureDir_Layers / "rope-Qwen3-0.6B-01.safetensor")
  ...
  runCppTest "RoPE owned by Model level — architectural decision":
    proc(): bool =
      ...
      let layer0_rotary = model.layers[0].sequence_mixer.rotary
      let layer1_rotary = model.layers[1].sequence_mixer.rotary
  ...
  runCppTest "Qwen3 RoPE cos/sin cache vs HF — end-to-end correctness":
    proc(): bool =
      ...
      var st = Safetensor.open(FixtureDirLongResidual / "block-00.safetensor")
