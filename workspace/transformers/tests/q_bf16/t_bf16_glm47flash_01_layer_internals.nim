...
## Layer-0 unit replay of the GLM-4.7-Flash checkpoint, one assertion
...
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_glm47flash_01_layer_internals.nim
...
const
  FixturePath = currentSourcePath().parentDir() / ".." / "fixtures" /
    ...
    "layer0-GLM-4.7-Flash-00.safetensor"
...
proc main() =
  ...
  let attn = setupMlaCompressed(ModelDir, Layer0Prefix, 0, 64, device = dev)
  ...
  block:
    var (orc, ctx) = newMlaCacheCtx(64, kvLoraRank, qkRopeHeadDim,
      @[1'u32, 2, 3, 4], device = dev)
    let cache = MlaLatentCache.init(kvLoraRank, qkRopeHeadDim, 64,
      kBFloat16, dev)
    let seqLen = 4
    let x = st.getTensorOwned("attn.input", dev)   # (1, 4, 2048) bf16
    ...
    assertStats(queryStates, StatsPath,
      "attn.query_states", kElementwise, depth = 1,
    ...
    assertStats(gotKpe, StatsPath, "attn.kpe_cached_interleaved",
      kElementwise, depth = 1, msg = "prefill kpe cache")
    ...
    assertStats(keyStates, StatsPath, "attn.key_states",
      kElementwise, depth = 1, msg = "prefill key states")
  ...
  # SwiGLU block, residual. The mixer consumes the engine rope rows.
  block:
  ...
  # and the routed output with the shared tail.
  block:
