...
## Greedy (temp=0) decoding of the gemma-4-12B-it stack, the recorded chains
...
## - the Dragon story prefill runs 554 prompt tokens through the production
##   KV path, the only chain crossing the 1024-token sliding window
##
## Requires the local model at tests/hf_models/gemma-4-12B-it (gitignored).
##
## Run:
##   nim cpp -r --hints:off --warnings:off --passC:"-std=c++20" \
##     --outdir:build/tests --nimcache:nimcache/tests workspace/transformers/tests/q_bf16/t_bf16_gemma412b_04_greedy_text_generation.nim
...
const
  ...
  FixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" /
    "bf16-04-greedy-text-generation" / "gemma-4-12B-it"
  GreedyFixtureFiles = [
    "The_sky_is_32_steps.json.zst",
    "To_be_or_not_to_be_that_is_32_steps.json.zst",
    "Dragon_story_crosses_1024_window_32_steps.json.zst",
  ]
...
proc main(): bool =
  ...
  for f in GreedyFixtureFiles:
    ...
    # prefill past the 1024-token sliding window stays inside it.
...
when isMainModule:
  runCppTest("gemma-4-12B greedy text generation", main)
