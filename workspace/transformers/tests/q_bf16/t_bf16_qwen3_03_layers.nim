...
const
  ...
  DecoderLayerFixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" / "layers" / "Qwen3-0.6B-block-8"
...
proc main() =
  ...
  runCppTest "RMSNorm layer fixtures":
    proc(): bool =
      ...
      let inputLn = RmsNorm.load(weights, cfgJson, "model.layers.8.input_layernorm")
      let postAttnLn = RmsNorm.load(weights, cfgJson, "model.layers.8.post_attention_layernorm")
  ...
  runCppTest "MLP layer fixtures":
    proc(): bool =
      ...
      let weights = SafetensorsCollection.open(ModelPath)
      let gateWeight = weights.getTensorOwned("model.layers.8.mlp.gate_proj.weight")
      let upWeight = weights.getTensorOwned("model.layers.8.mlp.up_proj.weight")
      let downWeight = weights.getTensorOwned("model.layers.8.mlp.down_proj.weight")

      let mlp = GatedDenseFFN.init(gateWeight, upWeight, downWeight, kSilu)
  ...
  runCppTest "Attention layer fixtures":
    proc(): bool =
      ...
      let weights = SafetensorsCollection.open(ModelPath)
      let qWeight = weights.getTensorOwned("model.layers.8.self_attn.q_proj.weight")
      let kWeight = weights.getTensorOwned("model.layers.8.self_attn.k_proj.weight")
      let vWeight = weights.getTensorOwned("model.layers.8.self_attn.v_proj.weight")
      let oWeight = weights.getTensorOwned("model.layers.8.self_attn.o_proj.weight")

      let cfgJson = (ModelDir / "config.json").parseFile()
      let qNorm = RmsNorm.load(weights, cfgJson, "model.layers.8.self_attn.q_norm")
      let kNorm = RmsNorm.load(weights, cfgJson, "model.layers.8.self_attn.k_norm")
      ...
      var attn = RopeGQAttention[RmsNorm].init(8, "model.layers.8.self_attn",
  ...
  runCppTest "Embedding + LMHead fixtures":
    proc(): bool =
      ...
      let weights = SafetensorsCollection.open(ModelPath)
      let embedWeight = weights.getTensorOwned("model.embed_tokens.weight")
  ...
  # running all items together. For now, process sequentially.
  # ──────────────────────────────────────────────────────────────────────────
  runCppTest "DecoderLayer fixtures":
    proc(): bool =
      # Load weights from main model (space-saving approach)

      let cfgJson = (ModelDir / "config.json").parseFile()
      let weights = SafetensorsCollection.open(ModelPath)
      let attn_norm = RmsNorm.load(weights, cfgJson, "model.layers.8.input_layernorm")
      let mlp_norm = RmsNorm.load(weights, cfgJson, "model.layers.8.post_attention_layernorm")
      let qNorm = RmsNorm.load(weights, cfgJson, "model.layers.8.self_attn.q_norm")
      let kNorm = RmsNorm.load(weights, cfgJson, "model.layers.8.self_attn.k_norm")
      let qWeight = weights.getTensorOwned("model.layers.8.self_attn.q_proj.weight")
      let kWeight = weights.getTensorOwned("model.layers.8.self_attn.k_proj.weight")
      ...
      var attn = RopeGQAttention[RmsNorm].init(8, "model.layers.8.self_attn",
        ...
        numQoHeads, numKvHeads, headDim, rotary,
        q_norm = qNorm, k_norm = kNorm)
      let mlp = GatedDenseFFN.init(Linear.init(gateWeight), Linear.init(upWeight), Linear.init(downWeight), kSilu)

      # Create the decoder block
      var transformer = DecoderLayer[RopeGQAttention[RmsNorm], GatedDenseFFN, RmsNorm].init(attn_norm, attn, mlp_norm, mlp)

      for caseNum in 0..3:
        let fixturePath = DecoderLayerFixtureDir / &"transformer-block-{ModelName}-{caseNum:02d}.safetensor"
        if not fileExists(fixturePath):
          continue
        ...
        assertAllClose(finalOutput, expectedOutput, rtol = 1e-5, abstol = 1e-5, msg = "DecoderLayer output case " & $caseNum & " failed")
        assertAllClose(finalOutputResidual, expectedOutputResidual, rtol = 1e-5, abstol = 1e-5, msg = "DecoderLayer output_residual case " & $caseNum & " failed")

        echo "DecoderLayer case ", caseNum, " PASSED"
      true
