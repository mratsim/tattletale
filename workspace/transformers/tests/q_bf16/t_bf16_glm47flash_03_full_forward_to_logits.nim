...
## Full-forward-to-logits suite for the GLM-4.7-Flash MoE stack.
## All 47 decoder blocks replay layer by layer over the long residual stream.
## Requires the local model at tests/hf_models/GLM-4.7-Flash (gitignored).
##
## Run:
##   nim cpp -r --hints:off --warnings:off --passC:"-std=c++20" \
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_glm47flash_03_full_forward_to_logits.nim
...
import
  ...
  workspace/transformers/src/models/glm47_flash {.all.},
...
privateAccess(Glm47Model)

const
  FixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" /
    "bf16-03-full-forward-to-logits" / "GLM-4.7-Flash"
  ModelPath = currentSourcePath().parentDir() / ".." / "hf_models" / "GLM-4.7-Flash"
...
proc main(): bool =
  ...
  let model = loadGlm47ModelRaw(ModelPath, F.kCPU)
  ...
  for layerIdx in 0 ..< model.layers.len:
    ...
    # Depth is the linear boundary count:
    # the block entry is the layer-indexed boundary, each prior block
    # contributing one drift class.
    if layerIdx == 0:
      ...
      assertStats(blockInput + stream.get(blockInput), statsPath,
        "layer_input_seq", kReduction, depth = layerIdx,
    ...
    # recorded on the last layer fixture, the depth is the full layer count
    # of the decoder stack.
    if layerIdx == model.layers.len - 1:
      assertStats(blockInput + stream.get(blockInput), statsPath,
        "layer_output_seq", kReduction, depth = model.layers.len,
  ...
  # The logits run one stage past the stack boundary, composing
  # the decoder stack, the final norm, and the head projection.
  for pos in 0 ..< logits.size(1):
    assertArgMax(logits.narrow(1, pos.int64, 1), decisionsPath, pos,
      kReduction, flipCount, depth = model.layers.len + 1,
      msg = "GLM-4.7-Flash final logits position " & $pos)
  result = true

when isMainModule:
  runCppTest("glm47flash full forward to logits", main)
