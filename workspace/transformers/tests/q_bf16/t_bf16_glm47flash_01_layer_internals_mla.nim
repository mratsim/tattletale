...
## Layer-0 compressed-Q MLA attention of the GLM-4.7-Flash checkpoint,
## replayed on cpu against the recorded layer-0 fixture payloads.
...
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_glm47flash_01_layer_internals_mla.nim
...
proc main() =
  ...
  block:
    ...
    let tablesStats = FixtureDir /
      "tables-GLM-4.7-Flash-00.safetensor.stats.json.zst"
  ...
  for caseNum in 0 ..< 3:
    let payload = FixtureDir /
      ("rope-GLM-4.7-Flash-0" & $caseNum & ".safetensor")
  ...
  for caseNum in 0 ..< 3:
    let payload = FixtureDir /
      ("norm-GLM-4.7-Flash-0" & $caseNum & ".safetensor")
    var st = Safetensor.open(payload)
    let statsPath = payload & ".stats.json.zst"
  ...
  for caseNum in 0 ..< 3:
    let payload = FixtureDir /
      ("qchain-GLM-4.7-Flash-0" & $caseNum & ".safetensor")
    var st = Safetensor.open(payload)
    let statsPath = payload & ".stats.json.zst"
    echo "    devices: ", deviceName(F.kCPU), " (qchain case ", caseNum, ")"
    let qAOut = attn.q_a_proj.forward(st.getTensorOwned("input"))
    assertStats(qAOut, statsPath, "q_a_out", kReduction,
      msg = "q_a projection, case " & $caseNum)
    let qNormed = attn.q_a_norm.forward(qAOut)
    assertStats(qNormed, statsPath, "q_normed", kElementwise,
      msg = "q bottleneck norm, case " & $caseNum)
    assertStats(attn.q_b_proj.forward(qNormed), statsPath, "q_out",
      kReduction, msg = "q_b projection, case " & $caseNum)

  # Prefill seq4 per-op replay. Every recomputed surface runs through
  ...
  block:
    ...
    var st = Safetensor.open(FixtureDir /
      "attn-GLM-4.7-Flash-00.safetensor")
    let statsPath = FixtureDir /
      "attn-GLM-4.7-Flash-00.safetensor.stats.json.zst"
    echo "    devices: ", deviceName(F.kCPU), " (prefill seq4)"
    let seqLen = 4
    let x = st.getTensorOwned("hidden_states")

    let q = attn.q_b_proj.forward(
      attn.q_a_norm.forward(attn.q_a_proj.forward(x))).reshape(
  ...
  block:
    let attnDev = setupMlaCompressed(ModelDir, LayerPrefix, 0, 64)
    var orc = newMlaOrchestrator(64, kvLoraRank, qkRopeHeadDim)
    orc.startSequence(@[1'u32, 2])
    var ctx = orc.getInferenceContextMut()
    var st = Safetensor.open(FixtureDir /
      "attn-GLM-4.7-Flash-01.safetensor")
    let statsPath = FixtureDir /
      "attn-GLM-4.7-Flash-01.safetensor.stats.json.zst"
    ...
    cache.write(ctx2, 0,
      st.getTensorOwned("latent_cached").reshape(
        [1, 3, 1, kvLoraRank]).narrow(1, 0, 2),
      st.getTensorOwned("kpe_cached_interleaved").reshape(
        [1, 3, 1, qkRopeHeadDim]).narrow(1, 0, 2),
      0, 2)
    orc2.setKvPosition(2)

    let xStep = st.getTensorOwned("x_step")
    let q = attnDev.q_b_proj.forward(
      attnDev.q_a_norm.forward(attnDev.q_a_proj.forward(xStep))).reshape(
      [1, 1, numHeads, qkNopeHeadDim + qkRopeHeadDim])
    let qPeRot = rotateInterleaved(q.narrow(3, qkNopeHeadDim, qkRopeHeadDim),
      st.getTensorOwned("cos_step"), st.getTensorOwned("sin_step"))
    assertStats(F.cat([q.narrow(3, 0, qkNopeHeadDim), qPeRot], 3), statsPath,
      "query_states_step", kElementwise, depth = 1,
  ...
  block:
    ...
    var ctx = orc.getInferenceContextMut()
    var st = Safetensor.open(FixtureDir /
      "attn-GLM-4.7-Flash-02.safetensor")
    let statsPath = FixtureDir /
      "attn-GLM-4.7-Flash-02.safetensor.stats.json.zst"
