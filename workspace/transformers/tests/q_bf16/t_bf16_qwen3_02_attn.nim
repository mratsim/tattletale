...
proc setupAttn(): RopeGQAttention[RmsNorm] =

  let cfgJson = (ModelDir / "config.json").parseFile()
  let weights = SafetensorsCollection.open(ModelPath)

  let qProj = Linear.load(weights, cfgJson, "model.layers.8.self_attn.q_proj")
  let kProj = Linear.load(weights, cfgJson, "model.layers.8.self_attn.k_proj")
  let vProj = Linear.load(weights, cfgJson, "model.layers.8.self_attn.v_proj")
  let oProj = Linear.load(weights, cfgJson, "model.layers.8.self_attn.o_proj")
  ...
  return RopeGQAttention[RmsNorm].init(
...
proc main() =
  ...
  runCppTest "KV cache stores rotated keys":
    proc(): bool =
      ...
      var st = Safetensor.open(FixtureDir / &"attn-{ModelName}-00.safetensor")
      ...
      let k_normed = attn.k_norm.forward(k_reshaped)
