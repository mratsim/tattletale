...
##     --outdir:build/tests --nimcache:nimcache/tests workspace/transformers/tests/q_bf16/t_bf16_laguna_03_full_forward_to_logits.nim
...
import
  ...
  workspace/transformers/src/models/laguna {.all.},
...
privateAccess(LagunaModel)

const
  FixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" /
    "bf16-03-full-forward-to-logits" / "Laguna-XS-2.1"
...
proc main(): bool =
  ...
  let model = loadLagunaModelRaw(ModelPath, runDev)
  ...
  # The embedding output carries no scale, the layer-0 recorded input.
  ...
  for layerIdx in 0 ..< model.layers.len:
    ...
    assertStats(nimInput, FixtureDir / ("layer-" & ($layerIdx).align(2, '0') & ".safetensor.stats"), "layer_input_seq", inputKind,
      depth = if layerIdx == 0: 1 else: 2, msg = "layer " & $layerIdx & " boundary input")
    ...
    if layerIdx < model.layers.len - 1:
      assertStats(nimSum, FixtureDir / ("layer-" & ($(layerIdx + 1)).align(2, '0') & ".safetensor.stats"), "layer_input_seq", kReduction, depth = 2, msg = "layer " & $layerIdx & " output plus residual, chained input")
    else:
      assertStats(nimSum, FixtureDir / ("layer-" & ($layerIdx).align(2, '0') & ".safetensor.stats"), "layer_output_seq", kReduction, depth = 2, msg = "layer " & $layerIdx & " output plus residual")
  ...
  for pos in 0 ..< decodedPositions:
    assertArgMax(finalLogits.narrow(1, pos.int64, 1), decisionsPath, pos,
      ...
      msg = "Laguna-XS-2.1 final logits position " & $pos)
  result = true

when isMainModule:
  runCppTest("laguna full forward to logits", main)
