...
## Full-forward-to-logits suite for the gemma-3-1b-it stack, the 26
...
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_gemma31b_03_full_forward_to_logits.nim
...
const
  FixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" /
    "bf16-03-full-forward-to-logits" / "gemma-3-1b-it"
...
proc main(): bool =
  ...
  for pos in 0 ..< decodedPositions:
    assertArgMax(finalLogits.narrow(1, pos.int64, 1), decisionsPath, pos,
      ...
      msg = "gemma-3-1b-it final logits position " & $pos)
  result = true

when isMainModule:
  runCppTest("gemma3 1b full forward to logits", main)
