...
## Layer-1 routed MoE block of the Moonlight-16B-A3B checkpoint, replayed
## on cpu against the recorded moe payloads (prefill seq6, decode pos8).
...
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_moonlight_01_layer_internals_moe.nim
...
proc main() =
  ...
  # the router constants arrive from the checkpoint config, the expert
  # stack and shared tail through the deserialization loader.
  ...
  # Per-payload replay. Case 0 carries the prefill seq6 pass, case 1
  # the decode pass at position 8, each payload its own stats frame.
  for caseNum in 0 ..< 2:
    let payload = FixtureDir / ("moe-Moonlight-16B-A3B-0" & $caseNum &
      ".safetensor")
    ...
    let hidden = st.getTensorOwned("hidden_states")
    let decision = router.route(hidden.reshape(
      [hidden.numel() div hiddenSize, hiddenSize]))
    ...
    # The decode pass reproduces the recorded single-rounding reduction
    # class. The prefill pass composes the eager weighted accumulation
    # over the expert body reduction, one depth step past the record.
    let outKind =
      if caseNum == 0: (kReduction, 2)
      else: (kReduction, 1)
    assertStats(moeOutput, statsPath, "moe_output", outKind[0],
      depth = outKind[1], msg = stage & " routed block output")
