...
## Greedy (temp=0) decoding of the gemma-3-1b-it stack, the recorded chains
...
## - the Fox story prefill runs 622 prompt tokens through the production
##   KV path, the only chain crossing the 512-token sliding window
##
## Requires the local model at tests/hf_models/gemma-3-1b-it (gitignored).
##
## Run:
##   nim cpp -r --hints:off --warnings:off --passC:"-std=c++20" \
##     --outdir:build/wip --nimcache:nimcache/wip workspace/transformers/tests/q_bf16/t_bf16_gemma31b_04_greedy_text_generation.nim
...
const
  ModelPath = currentSourcePath().parentDir() / ".." / "hf_models" / "gemma-3-1b-it"
  FixtureDir = currentSourcePath().parentDir() / ".." / "fixtures" /
    "bf16-04-greedy-text-generation" / "gemma-3-1b-it"
  GreedyFixtureFiles = [
    "Hello_how_are_you_32_steps.json.zst",
    "Big_blue_whales_eat_krill_32_steps.json.zst",
    "Fox_story_prefill_crosses_512_window_32_steps.json.zst",
  ]
...
when isMainModule:
  runCppTest("gemma3 1b greedy text generation", main)
