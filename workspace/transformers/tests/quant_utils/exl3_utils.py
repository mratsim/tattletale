"""
exl3_utils: shared logic for EXL3 fixture generation.

Function-name suffixes carry the role:

- `_orig_exl3`: thin shim around the EXL3 production kernel (ext.reconstruct)
- `_reimpl_exl3`: the project reimplementation of EXL3-internal behavior,
  the reference the fixture comparisons run against
- no suffix: pure utility (loading, parsing, shape derivation)

LOP3 note: the codebook decode formula ``(x & M1) ^ M2``, not the PTX-spec
``a ^ (b & c)``, reflects a PTX ↔ SASS index reversal in inline asm on sm_120:
...
def load_config(model_dir: str | None = None) -> dict:
    ...
    if model_dir is None:
        ...
        BASE_DIR = _d(_SCRIPT_DIR)  # the tests/ directory
...
def get_exl3_tensors(model_path: str) -> dict:
    ...
    Returns a dict keyed by safetensors tensor name:

    - each EXL3 layer key (for example `model.layers.0.self_attn.q_proj`)
      maps to its entry, the six keys `trellis`, `suh`, `svh`, `mcg`,
      `mul1`, `bias`, each holding a Tensor or None
    - `_norms`, `_embeddings`, `_lm_head`, `_others` map the remaining
      tensors, grouped by role
    """
    result: dict = {}
    norms: dict = {}
    embeddings: dict = {}
...
def linear_forward_orig_exl3(x: torch.Tensor, weight: torch.Tensor,
    ...
    **Weight layout**: ``weight`` must be ``[in_features, out_features]``
    ...
    Returns the layer output, shaped like ``x`` with the last dim widened
    to ``out_features``.
    ...
    # Input Hadamard (adds the suh scale, no extra norm, scale absorbs 1/sqrt(128))
...
def rms_norm_orig_exl3(x: torch.Tensor, weight: torch.Tensor,
    ...
    Returns the normed tensor, reshaped to the input shape.
    Requires compiled ``exllamav3_ext``.
    """
    from exllamav3.ext import exllamav3_ext as ext
    orig_shape = x.shape
...
def decode_codebook_reimpl_exl3(words: torch.Tensor, cb: int) -> torch.Tensor:
    ...
    **LOP3 note**: the CUDA source says ``lop3.b32 ... 0x6a`` but inline-asm
    on sm_120 executes LUT 0x78 (PTX ↔ SASS index reversal), so the correct
    formula ``(x & M1) ^ M2`` is verified empirically against the GPU kernel.
...
def reconstruct_reimpl_exl3(trellis: torch.Tensor, K: int, cb: int,
    ...
    - K: bitrate
    - cb: codebook variant, one of 0, 1, 2
    - out_shape: the (in_features, out_features) pair
    ...
    tile_size = 256
    packed_words = tile_size * K // 32  # trellis words per tile, uint32 units
...
def had_r_128_reimpl_exl3(x: torch.Tensor, scale: Optional[torch.Tensor] = None,
    ...
    - scale: optional [dim] element-wise scale
    - norm: post-transform normalization factor
    - pre_fwht: when True the scale applies before the FWHT, when False after it
    """
    x = x.clone()
    dim = x.shape[-1]
    with torch.no_grad():
...
def linear_forward_reimpl_exl3(x: torch.Tensor, weight: torch.Tensor,
    ...
    _norm = 0.088388347648  # 1/sqrt(128), matching the kernel-internal division
    xh = had_r_128_reimpl_exl3(x, suh, _norm)  # Input Hadamard: pre_fwht=True
    y = torch.nn.functional.linear(xh, weight)
    y = had_r_128_reimpl_exl3(y, svh, _norm, pre_fwht=False)  # output Hadamard, svh pass
...
def dequant_reimpl_exl3(w_reconstructed: torch.Tensor,
    ...
    Args:
    - w_reconstructed: [in_features, out_features] float16, the ``reconstruct_reimpl_exl3`` output
    - suh: [in_features] float16 input scale
    - svh: [out_features] float16 output scale
    - hadamard_dim: Hadamard block size, default 128

    Returns:
    - [in_features, out_features] float16 weight approximating the original
      FP16 linear weight
    """
    ...
    # Left: H_norm @ W (block-wise on rows, matching the exl3 preapply_had_l)
    for blk in range(0, k, HAD):
        w[blk:blk + HAD] = had_norm @ w[blk:blk + HAD]

    w *= suh.unsqueeze(1).float()  # suh row scale, matching the exl3 row order

    # Right: W @ H_norm (block-wise on cols, matching the exl3 preapply_had_r)
    for blk in range(0, n, HAD):
        w[:, blk:blk + HAD] = w[:, blk:blk + HAD] @ had_norm

    w *= svh.unsqueeze(0).float()  # svh column scale, matching the exl3 column order
