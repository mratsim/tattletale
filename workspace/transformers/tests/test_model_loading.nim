...
proc main() =
  ...
  runTest "Qwen3-0.6B forward pass (single token)":
    proc(): bool =
      ...
      let inputTensor = input.toTensor().unsqueeze(0)  # Shape: (1, 1)
      let positions = @[0'i64].toTensor().unsqueeze(0)  # Shape: (1, 1)
  ...
  runTest "Qwen3-0.6B forward pass (sequence)":
    proc(): bool =
      ...
      let inputTensor = input.toTensor().unsqueeze(0)  # Shape: (1, 3)
      let positions = @[0'i64, 1'i64, 2'i64].toTensor().unsqueeze(0)  # Shape: (1, 3)
  ...
  runTest "Qwen3-0.6B forward pass (batched)":
    proc(): bool =
      ...
      let inputTensor = input.toTensor().reshape([2, 3])  # Shape: (2, 3)
      let positions = @[0'i64, 1'i64, 2'i64, 0'i64, 1'i64, 2'i64].toTensor().reshape([2, 3])  # Shape: (2, 3)
  ...
  runTest "Qwen3-0.6B multiple forward passes (cache reset)":
    proc(): bool =
      ...
      let inputTensor1 = input1.toTensor().unsqueeze(0)
      let positions1 = @[0'i64, 1'i64].toTensor().unsqueeze(0)
      ...
      let inputTensor2 = input2.toTensor().unsqueeze(0)
      let positions2 = @[0'i64, 1'i64].toTensor().unsqueeze(0)
