...
const
  ...
  LogitDecisionsProbeSchema = "ttt-tf-002-logit-decisions-probe-h2"
    ## Format registry id of the v1 logit decisions test frame, one
    ## record per recorded position holding the decimal top-2 pair.
...
proc loadArgmaxDecisions(path: string): seq[ArgmaxRecord] =
  ## Parses one argmax decision frame in either recorded form
  ##
  ## Args:
  ...
  ## - the v1 schema carries JSON fields, a packed bracket string
  ##   for the top-2, the reduction kind
  ## Returns:
  ...
  of ArgmaxDecisionsSchema:
    ...
    for sj in items(j{"steps"}):
      ...
      result.add r
  of LogitDecisionsProbeSchema:
    for sj in items(j{"steps"}):
      var r = ArgmaxRecord(ulpDatatype: ulpBf16)
      r.argmaxId = sj{"argmax_id"}.getInt()
      r.margin = sj{"argmax_margin"}.getFloat()
      r.tailProbability = sj{"tail_probability"}.getFloat()
      let ids = sj{"top2_ids"}
      let logits = sj{"top2_logits"}
      if ids.kind == JString:
        for idj in items(parseJson(ids.getStr())):
          r.topK.add idj.getInt()
      else:
        for idj in items(ids):
          r.topK.add idj.getInt()
      if logits.kind == JString:
        for lj in items(parseJson(logits.getStr())):
          r.topKLogits.add lj.getFloat()
      else:
        for lj in items(logits):
          r.topKLogits.add lj.getFloat()
      r.kind = kReduction
      result.add r
  else:
    raise newException(ValueError,
      "decisions frame schema " & schema &
      " is neither " & ArgmaxDecisionsSchema & " nor " &
      LogitDecisionsProbeSchema)
