...
proc main() =
  ...
  # Step 9: MLP
  let gate = layer.hidden_mixer.gate_proj(h2); check("mlp_gate", gate, e_gate, maxDiff)
  let up = layer.hidden_mixer.up_proj(h2); check("mlp_up", up, e_up, maxDiff)
  let act = F.silu(gate) * up; check("mlp_activation", act, e_act, maxDiff)
  let down = layer.hidden_mixer.down_proj(act); check("mlp_down", down, e_down, maxDiff)
