...
from transformers.models.glm4_moe_lite.configuration_glm4_moe_lite import (  # noqa: E402
    Glm4MoeLiteConfig,
)
...
GEOMETRY.q_lora_rank = _CKPT_CONFIG["q_lora_rank"]
...
# search advances from SEED_MOE until the boundary margins clear the floor.
SEED_ATTN = 235
SEED_LAYER = 237
SEED_MOE = 236
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_glm47flash_01_layer_internals] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_glm47flash_01_layer_internals] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_glm47flash_01_layer_internals] free+inactive+speculative pool "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_glm47flash_01_layer_internals] other python/torch processes hold RAM: {stray}. "
...
def shard_of(key: str) -> str:
    ...
    if key not in weight_map:
        raise SystemExit(
            f"[gen_bf16_glm47flash_01_layer_internals] key {key} missing from the weight map")
...
def build_decoder_layer(weights: dict, cfg: Glm4MoeLiteConfig) -> Glm4MoeLiteDecoderLayer:
    """Glm4MoeLiteDecoderLayer with the real dense layer-0 weights loaded.

    Args:
    - weights, cfg, the load_checkpoint_tensors dict and the parsed config
    ...
    """
    layer = Glm4MoeLiteDecoderLayer(cfg, layer_idx=LAYER_IDX)
...
def build_routed_block(weights: dict, cfg: Glm4MoeLiteConfig) -> Glm4MoeLiteMoE:
    """Glm4MoeLiteMoE with the real first-routed-layer weights loaded.
    ...
    - the routed block in eval mode, the experts stacked from the per-expert
      checkpoint rows, each row carrying the SwiGLU projection pair in the chunked module layout
    """
    moe_layer_idx = GEOMETRY.first_k_dense_replace
    moe = Glm4MoeLiteMoE(cfg)
    prefix = f"model.layers.{moe_layer_idx}.mlp."
    with torch.no_grad():
        moe.gate.weight.data = weights[prefix + "gate.weight"]
        moe.gate.e_score_correction_bias.data = \
            weights[prefix + "gate.e_score_correction_bias"].float()
...
def generate_attn_mixture(layer, rotary) -> tuple:
    """Latent-cache attention prefill at seq 4, positions 0..3, the cache write of the full pass recorded.
    ...
    x = torch.randn(1, 4, GEOMETRY.hidden_size, dtype=torch.bfloat16)
    cap = run_pass(attn, rotary, x, list(range(4)), [])
    meta = {
        "case": "prefill_seq4",
        "seq_len": 4,
    ...
    print("[gen_bf16_glm47flash_01_layer_internals] attn mixture")
...
def generate_layer_mixture(layer, rotary) -> tuple:
    ...
    Args:
    - layer, rotary, the weighted decoder layer plus the reference rotary
    ...
    print("[gen_bf16_glm47flash_01_layer_internals] layer mixture "
          f"(seed {SEED_LAYER})")
    return meta, payload, captured
...
def generate_moe_mixture(moe) -> tuple:
    ...
    Args:
    - moe, the weighted layer-1 routed block

    Returns:
    - meta, the mixture metadata
    - payload, the single driving input tensor under its segment name
    - captured, the recorded decisions and block output for the stats frame

    The seed search advances one seed at a time until the top-k boundary
    margin and the smallest inner gap clear the floor on the sigmoid-plus-bias
    choice scores, the recorded floor of the exact expert-id comparisons.
    """
    ...
    for _ in range(64):
        ...
        margins = boundary_margins(router_logits, bias, top_k)
        if (margins["boundary_min"] > MARGIN_FLOOR
                and margins["inner_gap_min"] > MARGIN_FLOOR):
            break
        seed += 1
    else:
        raise SystemExit(
            "[gen_bf16_glm47flash_01_layer_internals] no margin-clean seed "
    ...
    meta = {
        ...
        "topk_boundary_margin_min": margins["boundary_min"],
        "topk_inner_gap_min": margins["inner_gap_min"],
    ...
    print(f"[gen_bf16_glm47flash_01_layer_internals] moe mixture (seed {seed}): "
          f"boundary margin {margins['boundary_min']:.3e}, inner gap min "
          f"{margins['inner_gap_min']:.3e}")
    return meta, payload, captured
...
def main() -> None:
    ...
    weights = load_checkpoint_tensors(
        LAYER0_TENSORS
        + [f"model.layers.{GEOMETRY.first_k_dense_replace}.mlp." + name
           for name in ["gate.weight", "gate.e_score_correction_bias",
                        "shared_experts.gate_proj.weight",
                        "shared_experts.up_proj.weight",
                        "shared_experts.down_proj.weight"]]
        + [f"model.layers.{GEOMETRY.first_k_dense_replace}.mlp.experts.{e}.{part}"
           for e in range(GEOMETRY.n_routed_experts)
    ...
    assert layer.self_attn.q_a_layernorm.variance_epsilon == 1e-6, (
    ...
    moe_meta, moe_payload, moe_captured = generate_moe_mixture(moe)
    ...
    print(f"[gen_bf16_glm47flash_01_layer_internals] torch {torch.__version__}, "
          f"transformers {transformers.__version__}")
    print(f"[gen_bf16_glm47flash_01_layer_internals] wrote {FIXTURE_PATH}")
