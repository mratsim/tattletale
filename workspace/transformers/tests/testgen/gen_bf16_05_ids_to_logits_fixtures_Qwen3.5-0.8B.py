...
Reference: gen_bf16_05_ids_to_logits_fixtures_Qwen3-0.6B.py conventions, extended with a
...
if not VENDORED_SRC or not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        "[gen_bf16_05_ids_to_logits_fixtures_Qwen3.5-0.8B] set QWEN35_VENDORED_SRC to the src directory of the pinned transformers "
...
def build_model(cfg: Qwen3_5Config) -> Qwen3_5ForConditionalGeneration:
    ...
    if len(missing) != 1 or missing[0] != "lm_head.weight":
        raise SystemExit(
            f"[gen_bf16_05_ids_to_logits_fixtures_Qwen3.5-0.8B] unexpected missing tensors: {missing}")
    if len(unexpected) != 15:
        raise SystemExit(
            f"[gen_bf16_05_ids_to_logits_fixtures_Qwen3.5-0.8B] unexpected foreign tensors: {unexpected}")
    return model
...
def replay_linear_layer(layer, layer_input):
    ...
    gdn_forward_replay = _load_sibling(
        "gen_bf16_02_gdn_fixtures_Qwen3.5-0.8B.py").gdn_forward_replay
...
def main() -> None:
    ...
    for i in range(num_layers):
        ...
        # in sub-ULP regime. The 0.00 contract is the sequential
