#!/usr/bin/env python3
"""Greedy-text-generation fixtures of the North-Mini-Code-1.0 checkpoint:
token chains argmax-decoded through the installed transformers modeling,
torch bf16, Metal (mps).

- fixture dir tests/fixtures/bf16-04-greedy-text-generation/North-Mini-Code-1.0/
- consumer tests/q_bf16/t_bf16_north_04_greedy_text_generation.nim
...
Every prompt runs the routed block, all 48 routed layers decode per step
through the eager expert loop.
...
Run from the worktree root
  ...
  uv run python workspace/transformers/tests/testgen/gen_bf16_north_04_greedy_text_generation.py
...
_ids = _load_sibling("gen_bf16_north_03_full_forward_to_logits.py")
...
# - prompt one reuses the exact token ids already recorded in the bf16-03
#   full-forward family, both fixture families share the deciding row
# - every prompt routes through all 48 routed layers of the checkpoint
...
PROMPT_SPEC = [
    ...
    ("Machine learning is fun", 32, "Machine_learning_is_fun_32_steps"),
]
MAX_HORIZON = 32
...
def main() -> None:
    ...
    for prompt_text, max_new_tokens, fixture_name in PROMPT_SPEC:
        ...
        fixture = {
            ...
            "env": recording_env(
                model=MODEL_NAME,
                generator="testgen/gen_bf16_north_04_greedy_text_generation.py",
