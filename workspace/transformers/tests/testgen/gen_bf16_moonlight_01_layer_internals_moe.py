...
DeepseekV3ForCausalLM with grouped_mm expert dispatch, on torch bf16:
- fixture dir tests/fixtures/bf16-01-layer-internals/Moonlight-16B-A3B-layer-1/
- consumer tests/q_bf16/t_bf16_moonlight_01_layer_internals_moe.nim

- moe-Moonlight-16B-A3B-<case>.safetensor, the recorded boundary slices of the case
...
Run from the worktree root
  uv run python workspace/transformers/tests/testgen/gen_bf16_moonlight_01_layer_internals_moe.py
"""
import argparse
import json
...
# 64 experts, top-6 routing, 2048-wide (no config parse in this script).
NUM_EXPERTS = 64
TOP_K = 6
HIDDEN = 2048
PREFILL_LEN = 8
...
def check_ram() -> None:
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_moonlight_01_layer_internals_moe] other python/torch processes hold RAM: {stray}. "
            "Stop and retry when idle")


def load_model() -> DeepseekV3ForCausalLM:
...
def main() -> None:
    ...
    assert m_pre["boundary_min"] > 0.0 and m_pre["inner_gap_min"] > 0.0, (
        "[gen_bf16_moonlight_01_layer_internals_moe] prefill margin ambiguity, "
        "pick a different seed")
    assert m_step["boundary_min"] > 0.0 and m_step["inner_gap_min"] > 0.0, (
        "[gen_bf16_moonlight_01_layer_internals_moe] decode margin ambiguity, "
        "pick a different seed")
    ...
    print(f"[gen_bf16_moonlight_01_layer_internals_moe] wrote {FIXTURE_DIR}")
    print(f"[gen_bf16_moonlight_01_layer_internals_moe] boundary margins "
