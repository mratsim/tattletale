#!/usr/bin/env python3
"""Tier-03 full-forward-to-logits fixture generator, gemma-4-E2B-it,
torch bf16 on Metal (mps) through the installed transformers modeling,

- consumer tests/q_bf16/t_bf16_gemma4e2b_03_full_forward_to_logits.nim
- embed through all 35 decoder layers plus the final norm and lm_head, one boundary slice layer-<i>.safetensor per layer
- the metadata rows carry the layer kind, the dual head dims and the kv-sharing row
...
- embed plus the PLE projection block, the final norm feeds the softcapped lm_head
- at the 5-token prompt both mask kinds skip to the sdpa is_causal path (mask None), the 512 window does not constrain it
...
Regenerate from the worktree root:
  ...
  uv run python workspace/transformers/tests/testgen/gen_bf16_gemma4e2b_03_full_forward_to_logits.py
"""
...
# - the 5-token prompt keeps the per-layer payloads inside
#   the 1.5 MiB family dir budget over the 35 layers
INPUT_TEXT = "The sky is blue today"
PROMPT_IDS = [818, 7217, 563, 3730, 3124]
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_gemma4e2b_03_full_forward_to_logits] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_gemma4e2b_03_full_forward_to_logits] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_gemma4e2b_03_full_forward_to_logits] free+inactive+speculative "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_gemma4e2b_03_full_forward_to_logits] other python/torch "
...
def build_model() -> Gemma4ForConditionalGeneration:
    ...
    Returns the model in eval mode, the text decoder layers carry
    the capture wrappers.
    """
    ...
    assert tcfg.layer_types[4] == "full_attention", (
        "layer 4 must be a full_attention row of the 4:1 pattern")
    ...
    assert tcfg.per_layer_config[0].head_dim == 256 and \
        tcfg.per_layer_config[4].head_dim == 512, (
...
def main() -> None:
    ...
    for i in range(num_layers):
        ...
        kv_shared = i >= tcfg.num_hidden_layers - tcfg.num_kv_shared_layers
        ...
        metadata = {
            ...
            "mask_note": "the 5-token prompt stays inside the 512 window, both "
        ...
        print(f"  layer {i:02d} ({layer_type}, head_dim "
              f"{metadata['head_dim']}, kv_shared {kv_shared}): {filepath}")
