...
through the installed transformers modeling on torch bf16:
- fixture dir tests/fixtures/bf16-04-greedy-text-generation/GLM-4.7-Flash/
- consumer tests/q_bf16/t_bf16_glm47flash_04_greedy_text_generation.nim
...
- expert dispatch at the grouped_mm default
...
- past a near-tie flip the consumer stops comparing that prompt
...
Run from the worktree root
  uv run python workspace/transformers/tests/testgen/gen_bf16_glm47flash_04_greedy_text_generation.py
"""
import argparse
import hashlib
import json
...
# The bf16-03-full-forward-to-logits generator carries the shared model checks,
# the zstd writer, the tokenizer loader and the identity constants, so
# both fixture generators verify the identical configuration:
# - determinism locks at import time in the sibling module through
#   one intra-op torch thread
_ids_inference = _load_sibling("gen_bf16_glm47flash_03_full_forward_to_logits.py")
...
write_json_zst = _ids_inference.write_json_zst
check_ram = _ids_inference.check_ram
build_model = _ids_inference.build_model
load_tokenizer = _ids_inference.load_tokenizer
index_census = _ids_inference.index_census
TRANSFORMERS_VERSION = _ids_inference.TRANSFORMERS_VERSION
...
# by this checkpoint's tokenizer:
# - prompt ids hard-asserted against the recorded corpus ids, verified
#   against the checkpoint tokenizer.json over the AutoTokenizer fast path
...
RECORDED_PROMPT_IDS = {
    "Hello, how are you?": [9703, 11, 1246, 525, 498, 30],
    "The capital of France is": [785, 6722, 315, 9621, 374],
    "Big blue whales eat krill.": [15624, 6303, 56392, 8179, 23040, 483, 13],
}
...
def generation_eos_ids() -> list[int]:
    """Stop ids from generation_config.json, the greedy chains assert none
    of these ids, every step stays a live argmax pick."""
...
def greedy_chain(model, token_ids: tuple[int, ...], max_new_tokens: int) -> dict:
    ...
    - the prefill covers the whole prompt and returns the past-key cache
    ...
    - exactly max_new_tokens argmax picks are recorded, the cache makes
      each step one O(1)-length forward
    """
    ...
    with torch.no_grad():
        ...
        for step in range(max_new_tokens):
            ...
            # torch.topk over the f32 logits records the top-32 support, and the argmax margin uses the top-2 values. torch.topk orders equal values
            # by a selection-internal detail that flips with k, while torch.argmax returns the first-max index. The recorded chosen_token stays
            # the argmax pick, an exact tie records margin 0.0, and the consumer checks classify the divergence at that step.
...
def prompt_fixture_name(text: str, max_new_tokens: int) -> str:
    """File name for one prompt fixture:
      alphanumerics only, horizon suffix, one file per prompt."""
...
def main() -> None:
    ...
    _ids_inference.index_census(weight_map)
    ...
    for prompt_text, max_new_tokens in PROMPT_SPEC:
        ...
        fixture = {
            ...
            "env": recording_env(
                model=MODEL_NAME,
                generator="testgen/gen_bf16_glm47flash_04_greedy_text_generation.py",
