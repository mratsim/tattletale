...
Reference: the Qwen3.5-0.8B layer-internals generator conventions.

What is generated (under tests/fixtures/bf16-01-layer-internals/Qwen3.5-0.8B-layer-3/):
...
Consumed by tests/q_bf16/t_bf16_unit_attn.nim (attn-*, norm-*) and
t_bf16_unit_rope.nim (rope-*), the per-op unit suites shared across the
dense ports. No Qwen3 analog: Qwen3.5 interleaves
this gated full-attention variant (per-head output gate, partial rotary)
with its GDN layers, and Qwen3 attention has neither gate nor partial
rotary, so the Qwen3-0.6B family records attention inside its full-layer
generator and carries no dedicated attention generator.
...
def load_layer3_weights() -> dict:
    """Load the six layer-3 self_attn tensors from the single safetensors
    file."""
...
def attention_forward_capture(attn, hidden_states, position_embeddings):
    ...
    # sdpa_attention_forward transposes back to batch, seq, heads, dim order
    # and makes result contiguous before reshape to batch, seq, -1 layout.
