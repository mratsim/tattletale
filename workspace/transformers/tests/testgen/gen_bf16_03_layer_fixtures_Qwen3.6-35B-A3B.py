...
What is generated (under tests/fixtures/layers/Qwen3.6-35B-A3B-layer-0/ and
tests/fixtures/layers/Qwen3.6-35B-A3B-layer-3/, the house layer dirs holding
one decoder layer's fixture files each):

  layer-Qwen3.6-35B-A3B-00.safetensor (+ .metadata.json.zip)
  ...
  layer-Qwen3.6-35B-A3B-03.safetensor (+ .metadata.json.zip)
...
Run (twice; cmp proves byte determinism):
  cd <worktree root> && .venv/bin/python \
    workspace/transformers/tests/testgen/gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B.py
...
if not VENDORED_SRC or not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        "[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] set QWEN36_VENDORED_SRC to the src directory of the pinned transformers "
...
# fixtures/layers/<Model>-layer-<N>/.
LAYERS_DIR = os.path.join(GRANDPARENT_DIR, "fixtures", "layers")
...
def vm_page_size() -> int:
    ...
    raise SystemExit("[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] vm_stat gave no page size line")
...
def free_bytes() -> int:
    ...
    for line in out.stdout.splitlines():
        if line.startswith("Pages free:"):
            ...
            return pages * page
    raise SystemExit("[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] vm_stat gave no 'Pages free' line")
...
def check_ram() -> None:
    ...
    if free < MIN_FREE_BYTES:
        raise SystemExit(
            f"[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] free memory {free / 1024 ** 3:.1f} GiB below "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] other python/torch processes hold RAM: {stray}; "
...
def check_vendored_sha() -> str:
    ...
    if sha != VENDORED_SHA:
        raise SystemExit(
            f"[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] vendored tree sha {sha} differs from "
...
def generate_layer0_fixture(cfg: Qwen3_5MoeTextConfig, weight_map: dict) -> None:
    ...
    for offset in range(64):
        ...
        raise SystemExit(
            "[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] no seed with unambiguous top-k "
    ...
    with torch.no_grad():
        ...
        assert torch.equal(module_mixer, chunk_replay["output"]), (
            "[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] chunked replay diverged from the "
        ...
        assert torch.equal(real_layer_out, manual_layer_out), (
            "[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] manual chain diverged from the "
        ...
        assert mixer_diff < BLOCK_BAR, (
            f"[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] recurrent-vs-chunked mixer diff "
...
def generate_layer3_fixture(cfg: Qwen3_5MoeTextConfig, weight_map: dict,
    ...
    with torch.no_grad():
        ...
        assert torch.equal(mixer_real, mixer_cap["output"]), (
            "[gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B] attention replay diverged from the "
...
def main() -> None:
    ...
    os.makedirs(LAYER0_DIR, exist_ok=True)
