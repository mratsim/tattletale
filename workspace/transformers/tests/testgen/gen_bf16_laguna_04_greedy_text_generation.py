#!/usr/bin/env python3
"""Greedy-text-generation fixtures of the Laguna-XS-2.1 checkpoint:
token chains argmax-decoded through the installed transformers modeling,
torch bf16, Metal (mps).

- fixture dir tests/fixtures/bf16-04-greedy-text-generation/Laguna-XS-2.1/
- consumer tests/q_bf16/t_bf16_laguna_04_greedy_text_generation.nim
...
- a chain pick that equals a configured eos id (2 or 24) fails the run,
  every recorded step stays a live argmax pick

The third prompt prefill-crosses the 512 sliding window.
...
- the 40-layer routed decode runs about 2 seconds per step on Metal, one
  32-step chain plus the weight load sits close to the execution tool's
  per-command timeout, so a chain can record one invocation at a time
- `--chain N` records only chain N (0-based), a bare run records all three

Run from the worktree root

  uv run python workspace/transformers/tests/testgen/gen_bf16_laguna_04_greedy_text_generation.py
...
_ids = _load_sibling("gen_bf16_laguna_03_full_forward_to_logits.py")
...
# - the dragon story repeats one sentence to a prefill past the 512
#   sliding window, its decode steps record the windowed-mask path
# - horizons stay inside 32 greedy steps
PROMPT_SPEC = [
    ("The sky is blue today", 32, "The_sky_is_blue_today_32_steps"),
    ("To be or not to be, that is", 32, "To_be_or_not_to_be_that_is_32_steps"),
    ("Once upon a time there was a small dragon. " * 55
     + "The dragon looked at", 32, "Dragon_story_crosses_512_window_32_steps"),
]
MAX_HORIZON = 32
...
def prompt_token_ids(tokenizer, text: str) -> tuple:
    """Prompt ids under the checkpoint tokenizer, the BOS id 2 prepended,
    the checkpoint tokenizer adds it."""
...
def main() -> None:
    ...
    for prompt_text, max_new_tokens, fixture_name in PROMPT_SPEC:
        chain_idx = PROMPT_SPEC.index(
            (prompt_text, max_new_tokens, fixture_name))
        ...
        if prompt_text == PROMPT_SPEC[2][0]:
            assert len(token_ids) > 512, (
                "the dragon-story prompt must tokenize past the 512 sliding "
        ...
        fixture = {
            ...
            "env": recording_env(
                model=MODEL_NAME,
                generator="testgen/gen_bf16_laguna_04_greedy_text_generation.py",
                extra={"dtype": "bfloat16", "device": "mps",
                       "num_threads": NUM_THREADS,
                       "attn_implementation": model.config
