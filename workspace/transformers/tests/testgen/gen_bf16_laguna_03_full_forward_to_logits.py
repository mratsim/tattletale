...
# AutoTokenizer over the shipped tokenizer files, BOS id 2 prepended:
# - the 6-token prompt keeps the per-layer payloads inside
#   the 1.5 MiB family dir budget over the 40 layers
INPUT_TEXT = "The sky is blue today"
PROMPT_IDS = [2, 785, 17059, 395, 6883, 6053]
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_laguna_03_full_forward_to_logits] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_laguna_03_full_forward_to_logits] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_laguna_03_full_forward_to_logits] free+inactive+speculative "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_laguna_03_full_forward_to_logits] other python/torch "
            f"processes hold RAM: {stray}, stop and retry when idle")


def load_tokenizer():
    """Tokenizes the prompt through the checkpoint tokenizer, the BOS id 2
    prepended (the checkpoint tokenizer adds it)."""
    return AutoTokenizer.from_pretrained(MODEL_DIR)


def build_model() -> LagunaForCausalLM:
    ...
    - full_attention layers at 0, 4, ... 36, sliding_attention between,
      window 512, 48 q heads on the full layers and 64 on the sliding
      layers with 8 kv heads everywhere
    - the dense prefix at layer 0, the routed block everywhere else, 256
      experts with top 8, the routed scaling factor 2.5
    """
    ...
    assert cfg.num_attention_heads_per_layer[0] == 48 \
        and cfg.num_attention_heads_per_layer[1] == 64, (
        "the recorded rows assume 48 q heads on the full layers and 64 on "
        "the sliding layers")
    assert cfg.num_key_value_heads == 8 and cfg.head_dim == 128, (
        "the recorded rows assume 8 kv heads over head_dim 128")
    assert cfg.mlp_layer_types[0] == "dense" and \
        all(t == "sparse" for t in cfg.mlp_layer_types[1:]), (
        "layer 0 must be the dense prefix, the routed block everywhere else")
...
def capture_forward(model: LagunaForCausalLM,
...
def main() -> None:
    ...
    for i in range(num_layers):
        ...
        metadata = {
            ...
            "mask_note": "the 6-token prompt stays inside the 512 window, both "
        ...
        print(f"  layer {i:02d} ({layer_type}, mlp {metadata['mlp_type']}, "
              f"heads {num_heads}): {filepath}")
