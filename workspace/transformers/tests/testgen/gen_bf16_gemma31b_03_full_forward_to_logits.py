...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_gemma31b_03_full_forward_to_logits] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_gemma31b_03_full_forward_to_logits] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_gemma31b_03_full_forward_to_logits] free+inactive+speculative pool "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_gemma31b_03_full_forward_to_logits] other python/torch processes hold RAM: {stray}, "
