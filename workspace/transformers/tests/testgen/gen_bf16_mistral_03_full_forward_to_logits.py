#!/usr/bin/env python3
"""Full-forward-to-logits fixtures of the Mistral-7B-v0.1 checkpoint,
recorded through the installed transformers modeling on torch bf16, Metal (mps).
The chain covers embed, all 32 decoder layers, the final norm and lm_head.

- fixture dir tests/fixtures/bf16-03-full-forward-to-logits/Mistral-7B-v0.1/
- consumer tests/q_bf16/t_bf16_mistral_03_full_forward_to_logits.nim

| file                                   | contents                                                                       |
| -------------------------------------- | ------------------------------------------------------------------------------ |
| layer-<i>.safetensor                   | the chain boundary slice layer_input_seq, the last layer also layer_output_seq |
| layer-<i>.safetensor.metadata.json.zst | the layer identity and the all-sliding shape rows                              |
...
At the 4-token prompt length the sliding mask skips to the sdpa is_causal
...
Run from the worktree root
  ...
  uv run python workspace/transformers/tests/testgen/gen_bf16_mistral_03_full_forward_to_logits.py
...
# - the short 4-token prompt keeps the per-layer payloads inside
#   the 1.5 MiB family dir budget over the 32 layers
INPUT_TEXT = "Hello there friend"
PROMPT_IDS = [1, 22557, 736, 1832]
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_mistral_03_full_forward_to_logits] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_mistral_03_full_forward_to_logits] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_mistral_03_full_forward_to_logits] free+inactive+speculative pool "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_mistral_03_full_forward_to_logits] other python/torch processes hold RAM: {stray}, "
            "stop and retry when idle")


def load_tokenizer():
    """Tokenizes the prompt through the checkpoint tokenizer.json, special
    tokens added (the BOS id 1 opens the recorded id list)."""
    return AutoTokenizer.from_pretrained(MODEL_DIR)


def build_model() -> MistralForCausalLM:
...
def capture_forward(model: MistralForCausalLM, input_ids: torch.Tensor) -> tuple:
...
def main() -> None:
    ...
    for i in range(num_layers):
        ...
        metadata = {
            ...
            "mask_note": "the 4-token prompt stays inside the 4096 window, the "
                "sliding mask skips to the sdpa is_causal path (mask None), the "
                "window behavior is recorded at tier 04",
        }
        filepath = save_fixture(f"layer-{i:02d}", metadata, tensors)
        print(f"  layer {i:02d} ({metadata['layer_type']}): {filepath}")
