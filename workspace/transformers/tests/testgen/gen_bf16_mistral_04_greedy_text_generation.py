#!/usr/bin/env python3
"""Greedy-text-generation fixtures of the Mistral-7B-v0.1 checkpoint, token
chains argmax-decoded through the installed transformers modeling on torch
bf16, recorded on Metal (mps).

- fixture dir tests/fixtures/bf16-04-greedy-text-generation/Mistral-7B-v0.1/
- consumer tests/q_bf16/t_bf16_mistral_04_greedy_text_generation.nim
...
The third prompt prefill-crosses the 4096 sliding window, 5042 prompt
tokens. The all-sliding layers record the windowed-mask path, the decode
steps run on the sliding mask over the grown cache.
...
Run from the worktree root
  ...
  uv run python workspace/transformers/tests/testgen/gen_bf16_mistral_04_greedy_text_generation.py
...
_ids = _load_sibling("gen_bf16_mistral_03_full_forward_to_logits.py")
...
# - the fox story repeats one sentence to 5042 prompt tokens, the prefill
#   crosses the 4096 sliding window
# - horizons stay inside 32 greedy steps
PROMPT_SPEC = [
    ("Hello there friend", 32, "Hello_there_friend_32_steps"),
    ("Big blue whales eat krill.", 32, "Big_blue_whales_eat_krill_32_steps"),
    ("The quick brown fox jumps over the lazy dog. " * 420, 32,
     "Fox_story_prefill_crosses_4096_window_32_steps"),
]
MAX_HORIZON = 32
...
def main() -> None:
    ...
    for prompt_text, max_new_tokens, fixture_name in PROMPT_SPEC:
        ...
        if prompt_text == PROMPT_SPEC[2][0]:
            assert len(token_ids) > 4096, (
                "the fox-story prompt must tokenize past the 4096 sliding "
        ...
        fixture = {
            ...
            "env": recording_env(
                model=MODEL_NAME,
                generator="testgen/gen_bf16_mistral_04_greedy_text_generation.py",
