...
What is generated (under tests/fixtures/ids-inference/Qwen3.6-35B-A3B/):

  layer-{i:02d}.safetensor (+ .metadata.json.zip), one per decoder layer i
  ...
  final_logits.safetensor (+ .metadata.json.zip)
...
accumulation formulation). The Nim ids-to-logits test asserts 0.00 against the
...
installed:
  cd <worktree root> && .venv/bin/python \
    workspace/transformers/tests/testgen/gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B.py
...
# bytes. QWEN36_VENDORED_SRC must point at its src directory. There is no
# default location.
VENDORED_SRC = os.environ.get("QWEN36_VENDORED_SRC")
if not VENDORED_SRC or not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        "[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] set "
        "QWEN36_VENDORED_SRC to the src directory of the pinned transformers "
        "checkout recorded in the fixtures metadata")
# The script directory first keeps the sibling generator imports working,
# VENDORED_SRC ahead of every installed copy makes `import transformers`
# resolve to the tree whose sha gates the fixture writes.
...
_layer_fixtures = _load_sibling("gen_bf16_03_layer_fixtures_Qwen3.6-35B-A3B.py")
...
FIXTURE_DIR = os.path.join(
    GRANDPARENT_DIR, "fixtures", "ids-inference", "Qwen3.6-35B-A3B"
)
...
def vm_page_size() -> int:
    ...
    raise SystemExit("[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit("[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] free+inactive+speculative pool "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] other python/torch processes hold RAM: {stray}; "
...
def check_vendored_sha() -> str:
    ...
    if sha != VENDORED_SHA:
        raise SystemExit(
            f"[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] vendored tree sha {sha} differs from "
...
def index_counts(weight_map: dict) -> None:
    ...
    assert total == expected, (
        f"[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] checkpoint index holds {total} keys, "
        f"expected {expected}")
...
def build_model(cfg, weight_map: dict) -> Qwen3_5MoeForConditionalGeneration:
    ...
    eval, CPU,
    through the vendored `from_pretrained` (per-file streaming, mmap-backed:
    ...
    if head.data_ptr() == embed.data_ptr():
        raise SystemExit(
            "[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] lm_head shares storage with embed_tokens; "
            "the untied head was silently tied")
    if torch.equal(head, embed):
        raise SystemExit(
            "[gen_bf16_05_ids_to_logits_fixtures_Qwen3.6-35B-A3B] lm_head equals embed_tokens elementwise; "
            "the untied head was silently tied")
...
def save_fixture(name: str, metadata: dict, tensors: dict) -> str:
    ...
    with open(filepath, "wb") as f:
        f.write(serialized)
    metadata_path = filepath + ".metadata.json.zip"
    write_json_zip(metadata_path, metadata)
    return filepath
...
def main() -> None:
    ...
    assert tokenizer_ids and len(tokenizer_ids) < 64, (
        "the ids-inference fixture prompt must stay inside one GDN chunk (64 tokens)")
    ...
    assert top_k == 8, (
        "the ids-inference fixtures pin the reference top-k of 8 experts")
    max_input_band = 0.0
    max_output_band = 0.0
    for i in range(num_layers):
        layer_obj = layers[i]
