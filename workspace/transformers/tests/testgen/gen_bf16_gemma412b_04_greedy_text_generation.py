#!/usr/bin/env python3
"""Greedy-text-generation fixtures of the gemma-4-12B-it checkpoint:
token chains argmax-decoded through the installed transformers modeling,
torch bf16, Metal (mps).

- fixture dir tests/fixtures/bf16-04-greedy-text-generation/gemma-4-12B-it/
...
- the 48-layer decode over the grown cache sits near the execution tool's
  per-command timeout once the weight load and the window-crossing prefill
  add up, so a chain can record one invocation at a time
