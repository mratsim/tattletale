#!/usr/bin/env python3
"""Greedy-text-generation fixtures of the gemma-3-270m-it checkpoint, token
chains argmax-decoded through the installed transformers modeling on torch
bf16, recorded on CPU.

- fixture dir tests/fixtures/bf16-04-greedy-text-generation/gemma-3-270m-it/
- consumer tests/q_bf16/t_bf16_gemma3270m_04_greedy_text_generation.nim
...
- a chain pick that equals a configured eos id fails the recording, every
  recorded step stays a live argmax pick

The third prompt prefill-crosses the 512 sliding window (622 prompt tokens).
The sliding_attention layers record the windowed-mask path, the decode
steps run on the bounded sliding cache.
...
Run from the worktree root
  ...
  uv run python workspace/transformers/tests/testgen/gen_bf16_gemma3270m_04_greedy_text_generation.py
...
_ids = _load_sibling("gen_bf16_gemma3270m_03_full_forward_to_logits.py")
...
# - two short prompts, each tokenized to at most 8 tokens by this tokenizer
# - the fox story repeats one sentence to 622 prompt tokens, the prefill
#   crosses the 512 sliding window
# - horizons stay inside 32 greedy steps, prompt one reuses the exact token
#   ids already recorded in the bf16-03 full-forward family, both fixture
#   families share the deciding row
PROMPT_SPEC = [
    ("Hello, how are you?", 32, "Hello_how_are_you_32_steps"),
    ("Big blue whales eat krill.", 32, "Big_blue_whales_eat_krill_32_steps"),
...
def prompt_token_ids(tokenizer, text: str) -> tuple:
    """Prompt ids under the checkpoint tokenizer, special tokens added so
    the BOS id 2 opens the list."""
...
def greedy_chain(model, token_ids: tuple, max_new_tokens: int) -> dict:
    ...
    - token_ids, max_new_tokens, the prompt ids and the chain horizon
...
def main() -> None:
    ...
    for prompt_text, max_new_tokens, fixture_name in PROMPT_SPEC:
        ...
        if prompt_text == PROMPT_SPEC[2][0]:
            assert len(token_ids) > 512, (
                "the fox-story prompt must tokenize past the 512 sliding "
        ...
        fixture = {
            ...
            "env": recording_env(
                model=MODEL_NAME,
                generator="testgen/gen_bf16_gemma3270m_04_greedy_text_generation.py",
