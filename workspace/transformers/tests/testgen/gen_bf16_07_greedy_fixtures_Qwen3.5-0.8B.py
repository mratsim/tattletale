...
Reference: gen_bf16_07_greedy_fixtures_Qwen3-0.6B.py conventions (one JSON file per prompt).
...
if not VENDORED_SRC or not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        "[gen_bf16_07_greedy_fixtures_Qwen3.5-0.8B] set QWEN35_VENDORED_SRC to the src directory of the pinned transformers "
...
def build_model(cfg: Qwen3_5Config) -> Qwen3_5ForConditionalGeneration:
    ...
    if len(missing) != 1 or missing[0] != "lm_head.weight":
        raise SystemExit(
            f"[gen_bf16_07_greedy_fixtures_Qwen3.5-0.8B] unexpected missing tensors: {missing}")
    if len(unexpected) != 15:
        raise SystemExit(
            f"[gen_bf16_07_greedy_fixtures_Qwen3.5-0.8B] unexpected foreign tensors: {unexpected}")
    return model
