...
_ids = _load_sibling("gen_bf16_gptoss_03_full_forward_to_logits.py")
...
# - the dragon story repeats one sentence to a prefill past the 128
...
PROMPT_SPEC = [
    ...
    ("Once upon a time there was a small dragon. " * 13
     + "The dragon looked at", 32, "Dragon_story_crosses_128_window_32_steps"),
]
MAX_HORIZON = 32
...
def main() -> None:
    ...
    for prompt_text, max_new_tokens, fixture_name in PROMPT_SPEC:
        ...
        if prompt_text == PROMPT_SPEC[2][0]:
            assert len(token_ids) > model.config.sliding_window, (
                "the dragon-story prompt must tokenize past the 128 sliding "
        ...
        fixture = {
            ...
            "env": recording_env(
                model=MODEL_NAME,
                generator="testgen/gen_bf16_gptoss_04_greedy_text_generation.py",
                extra={"dtype": "bfloat16", "device": "mps",
                       "num_threads": NUM_THREADS,
                       "attn_implementation": model.config
                       ._attn_implementation,
                       "weight_note": "the checkpoint ships MXFP4 expert "
                       "weights, the reference quantizer dequantizes them "
                       "to bf16 at load (mps has no MXFP4 kernel)"}),
            "prompt": prompt_text,
