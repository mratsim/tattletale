...
MLA layer-0 records (tables-*, norm-*, rope-*, attn-*) through the installed
reference modeling, DeepseekV3Attention with the sdpa interface, on torch bf16:
- fixture dir tests/fixtures/bf16-01-layer-internals/Moonlight-16B-A3B-layer-0/
- consumer tests/q_bf16/t_bf16_moonlight_01_layer_internals_mla.nim

- <name>-Moonlight-16B-A3B-<case>.safetensor, the recorded boundary slices of the case
...
- bf16-spelled interleaved rope with the interleaved-layout data-movement twins
...
Run from the worktree root
  uv run python workspace/transformers/tests/testgen/gen_bf16_moonlight_01_layer_internals_mla.py
"""
import argparse
import json
...
# the 16/512/128/64/128/2048 tuple the checkpoint carries.
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_moonlight_01_layer_internals_mla] vm_stat gave no page size line")
...
def free_bytes() -> int:
    ...
    for line in out.stdout.splitlines():
        if line.startswith("Pages free:"):
            ...
            return pages * page
    raise SystemExit(
        "[gen_bf16_moonlight_01_layer_internals_mla] vm_stat gave no 'Pages free' line")
...
def check_ram() -> None:
    ...
    if free < MIN_FREE_BYTES:
        raise SystemExit(
            f"[gen_bf16_moonlight_01_layer_internals_mla] free memory {free / 1024 ** 3:.1f} GiB below "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_moonlight_01_layer_internals_mla] other python/torch processes hold RAM: {stray}. "
...
def shard_of(key: str) -> str:
    ...
    if key not in weight_map:
        raise SystemExit(
            f"[gen_bf16_moonlight_01_layer_internals_mla] key {key} missing from the weight map")
    return weight_map[key]


def load_config() -> DeepseekV3Config:
    """Checkpoint config.json load, flat DeepseekV3 layout:
    - rope_parameters defaults to the plain theta spelling, keeping
      rope_type default, theta 50000 and rope_interleave true
    - attn implementation locked to sdpa"""
    with open(CONFIG_PATH) as f:
...
def load_layer0_weights() -> dict:
    """Load the five layer-0 self_attn tensors from the shards that hold them, memory-mapped safe_open reads."""
    weights = {}
...
def build_attention(weights: dict, cfg: DeepseekV3Config) -> DeepseekV3Attention:
    """DeepseekV3Attention with real layer-0 weights loaded."""
    attn = DeepseekV3Attention(cfg, layer_idx=LAYER_IDX)
    with torch.no_grad():
...
def generate_tables_fixtures(rotary: DeepseekV3RotaryEmbedding) -> None:
...
def generate_rope_fixtures(rotary: DeepseekV3RotaryEmbedding) -> None:
...
def attention_forward_capture(attn, hidden_states, cos, sin, cache):
    """Replay of the reference DeepseekV3Attention.forward with intermediate capture, the reference forward body op for op on the sdpa
    interface:
    - q_proj split into q_pass and q_rot
    - kv_a_proj_with_mqa split into the latent and the raw plane
    - latent norm, the interleaved rotation on the bf16 tables
    - cache update, expand_kv, torch SDPA with the module scaling, o_proj
    The caller asserts the replayed output equals the module's own forward output."""
...
def generate_attn_fixtures(attn: DeepseekV3Attention,
                           rotary: DeepseekV3RotaryEmbedding) -> None:
    ...
    cap = run_pass(attn, x, list(range(8)), [])
    save_fixture(
        layer_name, 0,
        {
            **COMMON_META,
    ...
    cap_step = run_pass(attn, x_step, [3], cache_layers)
    save_fixture(
        layer_name, 1,
        {
            **COMMON_META,
            "case": "prefill3_decode1_pos3",
            "positions": [0, 1, 2],
            "positions_step": [3],
            "seed": SEED_ATTN + 1,
            "softmax_scaling": 0.07216878364870322,
...
def main() -> None:
    ...
    print(f"[gen_bf16_moonlight_01_layer_internals_mla] torch {torch.__version__}, "
          f"transformers {transformers.__version__}")
    print(f"[gen_bf16_moonlight_01_layer_internals_mla] wrote {FIXTURE_DIR}")
