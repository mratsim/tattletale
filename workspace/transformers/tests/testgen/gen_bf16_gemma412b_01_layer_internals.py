...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_gemma412b_01_layer_internals] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_gemma412b_01_layer_internals] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    - the free+inactive+speculative pool sits above the 48 GiB floor
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_gemma412b_01_layer_internals] free+inactive+speculative "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_gemma412b_01_layer_internals] other python/torch processes "
...
def load_model() -> Gemma4ForConditionalGeneration:
    ...
    Identity asserts of the checkpoint config, dual-dim k_eq_v shape:

| property      | recorded value                                                    |
| ------------- | ----------------------------------------------------------------- |
| layer pattern | 5 sliding_attention layers then 1 full_attention, window 1024     |
| head dims     | sliding 256 over 8 kv heads, full 512 over 1 kv head              |
| attention     | attention_k_eq_v on the full layers, v_proj None, unscaled v_norm |
| block         | no kv sharing, no PLE, dense mlp everywhere                       |
    """
    ...
    assert tcfg.per_layer_config[FULL_LAYER_IDX].num_key_value_heads == 1, (
        "the recorded rows assume the full layers run 1 kv head")
    ...
    assert tcfg.enable_moe_block is False and tcfg.use_double_wide_mlp is False, (
        "the recorded rows assume the dense mlp, no routed block")
    ...
    assert tcfg.num_hidden_layers == 48 and tcfg.hidden_size == 3840 \
        and tcfg.vocab_size == 262144, (
        "the recorded rows assume the 48-layer, 3840-hidden, 262144-vocab checkpoint")
...
def attention_capture(attn, h_norm: torch.Tensor, cos: torch.Tensor,
    ...
    assert torch.equal(module_output, output), (
        "[gen_bf16_gemma412b_01_layer_internals] manual attention replay "
...
def cache_boundary_rows(attn, k: torch.Tensor, v: torch.Tensor,
    ...
    assert torch.equal(stored_k, k) and torch.equal(stored_v, v), (
        "[gen_bf16_gemma412b_01_layer_internals] the cache update must store "
...
def layer_chain_capture(layer, x: torch.Tensor, cos: torch.Tensor,
    ...
    - the capture dict, every norm output, the attention rows, the mlp rows,
      the pre-scale layer sum and the scaled layer output
...
is the residual sum times `layer.layer_scalar`.
    ...
    with torch.no_grad():
        ...
        layer_out = h3 * layer.layer_scalar
        module_out = layer(
    ...
    assert torch.equal(module_out, layer_out), (
        "[gen_bf16_gemma412b_01_layer_internals] manual layer chain diverged "
...
def generate_layer0_mixture(model: Gemma4ForConditionalGeneration,
    ...
    print(f"[gen_bf16_gemma412b_01_layer_internals] layer0 mixture "
          f"(seed {SEED_LAYER0})")
...
def generate_layer5_mixture(model: Gemma4ForConditionalGeneration,
    ...
    k projection row and cache boundary over one seeded input.
    ...
    x = seeded_input(SEED_LAYER5, (1, SEQ, tcfg.hidden_size))
    pos_ids = torch.arange(SEQ, device="mps").unsqueeze(0)
    cos, sin = lm.rotary_emb(x, pos_ids, layer_type="full_attention")
    mask = build_mask(tcfg, x, pos_ids, tcfg.layer_types[FULL_LAYER_IDX])
    cap = layer_chain_capture(layer, x, cos, sin, mask, pos_ids)
    attn = layer.self_attn
    cap.update(cache_boundary_rows(attn, cap["k_rot"], cap["v"], FULL_LAYER_IDX))
    meta = {
        "case": "prefill_seq6_k_eq_v",
    ...
    print(f"[gen_bf16_gemma412b_01_layer_internals] layer5 mixture "
          f"(seed {SEED_LAYER5}, k_eq_v)")
    return meta, payload, captured
...
def main() -> None:
    ...
    metadata = {
        ...
        "attention_shape": "5 sliding_attention layers then 1 full_attention "
            ...
            "heads and full head_dim 512 over 1 kv head, no kv sharing",
    ...
    print(f"[gen_bf16_gemma412b_01_layer_internals] torch {torch.__version__}, "
          f"transformers {transformers.__version__}")
    print(f"[gen_bf16_gemma412b_01_layer_internals] wrote {FIXTURE_PATH}")
