#!/usr/bin/env python3
"""GLM-4.7-Flash fixture generator over the real checkpoint shards, resolved
through the gitignored tests/hf_models symlink.

bf16-03 full-forward-to-logits records, the full embed -> 47 decoder layers ->
norm -> lm_head chain through installed transformers modeling on torch bf16:
- fixture dir tests/fixtures/bf16-03-full-forward-to-logits/GLM-4.7-Flash/
- consumer tests/q_bf16/t_bf16_glm47flash_03_full_forward_to_logits.nim
...
Run from the worktree root
  uv run python workspace/transformers/tests/testgen/gen_bf16_glm47flash_03_full_forward_to_logits.py
"""
import argparse
import copy
import json
...
_moe_fixtures = _load_sibling("gen_bf16_glm47flash_01_layer_internals_moe.py")
...
PROMPT_IDS = [9703, 11, 1246, 525, 498, 30]
...
# - 9703 tensors on disk, the draft block (model.layers.47.*) carries 212
#   of them and never loads, transformers excludes those keys itself
# - loader scope stays the main stack 0..46 plus the untied lm_head.weight
TOTAL_KEYS = 9703
MTP_KEYS = 212
MTP_LAYER = 47
NUM_LAYERS = 47
...
# Eager-vs-grouped_mm band guards, both expert formulations agree on one
# bf16-grid routed block, the class divergence accumulates through 46
# routed layers of residual stream:
# - one routed block measured worst 2^-9 = 1 bf16 ulp on the bf16-02 MoE-internals rows
# - committed GLM sidecars measured the accumulated input band, 0.25
#   at layer 13, 0.5 from layer 16, 1.0 from layer 39, 2.0 at layers
#   41-46, the output band up to 3.0 at the last layer
# the guards carry headroom over those worst bands, they catch gross
...
# - the input guard 4.0 carries 2x the GLM input worst 2.0
# - the output guard 12.0 carries 4x the 3.0 worst
# - the logits band 16.0 carries 8x the 2.0 worst
...
def vm_page_size() -> int:
    ...
    raise SystemExit("[gen_bf16_glm47flash_03_full_forward_to_logits] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit("[gen_bf16_glm47flash_03_full_forward_to_logits] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_glm47flash_03_full_forward_to_logits] free+inactive+speculative pool "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_glm47flash_03_full_forward_to_logits] other python/torch processes hold RAM: {stray}, "
            "stop and retry when idle")


def index_census(weight_map: dict) -> None:
    """Check the checkpoint index against the recorded counts, 9703 tensors, all under model.* except the untied lm_head.weight."""
    total = len(weight_map)
    assert total == TOTAL_KEYS, (
        f"[gen_bf16_glm47flash_03_full_forward_to_logits] checkpoint index holds {total} keys, "
    ...
    mtp = sum(1 for key in weight_map
              if key.startswith(f"model.layers.{MTP_LAYER}."))
    assert mtp == MTP_KEYS, (
        f"draft block holds {mtp} keys, expected {MTP_KEYS}")


def build_model(weight_map: dict) -> Glm4MoeLiteForCausalLM:
    ...
    model.eval()
    # Router selection bias stays at its checkpoint f32 bytes:
    # - the strict keep-in-fp32 policy of the reference class applies even
    #   on a bf16 deploy, the measured dtype plan keeps the selection-bias
    #   buffer in torch.float32, the live buffer equals the checkpoint
    #   values on all 64/64 entries at layer 1
    # - blanket post-load dtype casts silently regrid the selection score,
    #   the per-op MoE fixtures and the Nim loader both keep the checkpoint f32 grid
    for i in range(FIRST_K_DENSE_REPLACE, NUM_LAYERS):
        bias = model.model.layers[i].mlp.gate.e_score_correction_bias
        assert bias.dtype == torch.float32, (
            f"layer {i} selection bias drifted off the checkpoint f32 "
            f"grid, dtype {bias.dtype!r} breaks the recorded chain")
    ...
    if head.data_ptr() == embed.data_ptr():
        raise SystemExit(
            "[gen_bf16_glm47flash_03_full_forward_to_logits] lm_head shares storage with embed_tokens, "
            "the untied head was silently tied")
    if torch.equal(head, embed):
        raise SystemExit(
            "[gen_bf16_glm47flash_03_full_forward_to_logits] lm_head equals embed_tokens elementwise, "
            "the untied head was silently tied")
...
def main() -> None:
    ...
    # Decision projection of the recorded grouped_mm run, the raw logits tensor leaves the tree, the consumers carry the argmax id, the top-32
    # support and the tail probability of each recorded position. The eager-vs-grouped_mm band stays as the recorded metadata band.
