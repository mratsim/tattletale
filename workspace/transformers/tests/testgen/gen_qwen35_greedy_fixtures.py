...
if not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        f"[gen_qwen35_greedy_fixtures] vendored modeling not found at {VENDORED_SRC}. "
...
def build_model(cfg: Qwen3_5Config) -> Qwen3_5ForConditionalGeneration:
    ...
    if len(missing) != 1 or missing[0] != "lm_head.weight":
        raise SystemExit(
            f"[gen_qwen35_greedy_fixtures] unexpected missing tensors: {missing}")
    if len(unexpected) != 15:
        raise SystemExit(
            f"[gen_qwen35_greedy_fixtures] unexpected foreign tensors: {unexpected}")
    return model
