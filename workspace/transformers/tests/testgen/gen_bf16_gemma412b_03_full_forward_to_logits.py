#!/usr/bin/env python3
"""Full-forward-to-logits fixtures of the gemma-4-12B-it checkpoint,
recorded through the installed transformers modeling on torch bf16, Metal (mps).
The chain covers embed, all 48 decoder layers, the final norm and the softcapped lm_head.

- fixture dir tests/fixtures/bf16-03-full-forward-to-logits/gemma-4-12B-it/
- consumer tests/q_bf16/t_bf16_gemma412b_03_full_forward_to_logits.nim
...
The 3-token prompt stays inside the 1.5 MiB family dir budget over the 48 layers at the 3840
hidden width. At that length both mask kinds skip to the sdpa is_causal path (mask None)
and the 1024-token window does not constrain the rows, tier-04 carries the window behavior.
...
Run from the worktree root
  ...
  uv run python workspace/transformers/tests/testgen/gen_bf16_gemma412b_03_full_forward_to_logits.py
...
# - the 3-token prompt keeps the per-layer payloads inside
#   the 1.5 MiB family dir budget over the 48 layers at hidden 3840
INPUT_TEXT = "The sky is"
PROMPT_IDS = [818, 7217, 563]
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_gemma412b_03_full_forward_to_logits] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_gemma412b_03_full_forward_to_logits] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_gemma412b_03_full_forward_to_logits] free+inactive+speculative "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_gemma412b_03_full_forward_to_logits] other python/torch "
...
def main() -> None:
    ...
    for i in range(num_layers):
        ...
        metadata = {
            ...
            "mask_note": "the 3-token prompt stays inside the 1024 window, both "
