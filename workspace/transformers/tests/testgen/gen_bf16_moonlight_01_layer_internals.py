#!/usr/bin/env python3
"""Tier-01 layer-0 fixture generator, Moonlight-16B-A3B,
CPU torch bf16, installed reference modeling from safetensors,

- consumer tests/q_bf16/t_bf16_moonlight_01_layer_internals.nim
- one bare bf16 driving tensor per mixture (attn.input, layer.layer_input, moe.h), every recorded intermediate a stats fingerprint
- the mixtures sit under first_k_dense_replace, the attention mixer op surface on the prefill path, the dense layer-0 chain

- the moe mixture records the routed block surface of layer 1 on a margin-clean input

- the routed mixture stands on a margin-clean seed, the top-k boundary margin clears the 1e-4 sigmoid-plus-bias choice floor
