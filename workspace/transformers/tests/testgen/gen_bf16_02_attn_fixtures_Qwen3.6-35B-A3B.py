...
fixtures from the real checkpoint safetensors files, using the vendored
reference
modeling on CPU torch bf16.

What is generated (under tests/fixtures/layers/Qwen3.6-35B-A3B-layer-3/):
...
Run (twice; cmp proves byte determinism):
  cd <worktree root> && .venv/bin/python \
    workspace/transformers/tests/testgen/gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B.py
...
if not VENDORED_SRC or not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        "[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] set QWEN36_VENDORED_SRC to the src directory of the pinned transformers "
...
FIXTURE_DIR = os.path.join(
    GRANDPARENT_DIR, "fixtures", "layers", "Qwen3.6-35B-A3B-layer-3"
)
...
def vm_page_size() -> int:
    ...
    raise SystemExit("[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] vm_stat gave no page size line")
...
def free_bytes() -> int:
    ...
    for line in out.stdout.splitlines():
        if line.startswith("Pages free:"):
            ...
            return pages * page
    raise SystemExit("[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] vm_stat gave no 'Pages free' line")
...
def check_ram() -> None:
    ...
    if free < MIN_FREE_BYTES:
        raise SystemExit(
            f"[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] free memory {free / 1024 ** 3:.1f} GiB below "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] other python/torch processes hold RAM: {stray}; "
...
def check_vendored_sha() -> str:
    ...
    if sha != VENDORED_SHA:
        raise SystemExit(
            f"[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] vendored tree sha {sha} differs from "
...
def main() -> None:
    ...
    print(f"[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] torch {torch.__version__}, vendored sha {sha[:12]}")
    print(f"[gen_bf16_02_attn_fixtures_Qwen3.6-35B-A3B] wrote {FIXTURE_DIR}")
