#!/usr/bin/env python3
"""Boundary-pair fixture file of the gemma-3-1b-it checkpoint, recorded
...
| file                                                   | contents                                                                             |
| ------------------------------------------------------ | ------------------------------------------------------------------------------------ |
| layer4-5-gemma-3-1b-it-00.safetensor                   | sliding.input, full.input, boundary.input                                            |
| layer4-5-gemma-3-1b-it-00.safetensor.metadata.json.zst | per-mixture metadata under the mixtures key                                          |
| layer4-5-gemma-3-1b-it-00.safetensor.stats.json.zst    | one uniform record per recorded tensor, keys namespaced sliding. / full. / boundary. |
...
Consumed by tests/q_bf16/t_bf16_gemma31b_01_layer_internals.nim, one
assertion block per mixture.

Run from the worktree root

  uv run python workspace/transformers/tests/testgen/gen_bf16_gemma31b_01_layer_internals.py
...
def vm_page_size() -> int:
    ...
    raise SystemExit(
        "[gen_bf16_gemma31b_01_layer_internals] vm_stat gave no page size line")
...
def pool_bytes() -> int:
    ...
    if pool == 0:
        raise SystemExit(
            "[gen_bf16_gemma31b_01_layer_internals] vm_stat gave no pool lines")
    return pool
...
def check_ram() -> None:
    ...
    if pool < MIN_POOL_BYTES:
        raise SystemExit(
            f"[gen_bf16_gemma31b_01_layer_internals] free+inactive+speculative pool "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_gemma31b_01_layer_internals] other python/torch processes hold RAM: {stray}, "
...
def attention_capture(attn, h_norm: torch.Tensor, cos: torch.Tensor,
    ...
    assert torch.equal(module_output, output), (
        "[gen_bf16_gemma31b_01_layer_internals] manual attention replay "
...
def layer_chain_capture(layer, x: torch.Tensor, cos: torch.Tensor,
    ...
    assert torch.equal(module_out, layer_out), (
        "[gen_bf16_gemma31b_01_layer_internals] manual layer chain diverged "
...
def generate_sliding_mixture(model: Gemma3ForCausalLM, cfg) -> tuple:
    ...
    print("[gen_bf16_gemma31b_01_layer_internals] sliding mixture "
          f"(seed {SEED_SLIDING})")
...
def generate_full_mixture(model: Gemma3ForCausalLM, cfg) -> tuple:
    ...
    print("[gen_bf16_gemma31b_01_layer_internals] full mixture "
          f"(seed {SEED_FULL})")
    return meta, payload, captured
...
def generate_boundary_mixture(model: Gemma3ForCausalLM, cfg) -> tuple:
    ...
    print("[gen_bf16_gemma31b_01_layer_internals] boundary mixture "
...
def main() -> None:
    ...
    print(f"[gen_bf16_gemma31b_01_layer_internals] torch {torch.__version__}, "
          f"transformers {transformers.__version__}")
    print(f"[gen_bf16_gemma31b_01_layer_internals] wrote {FIXTURE_PATH}")
