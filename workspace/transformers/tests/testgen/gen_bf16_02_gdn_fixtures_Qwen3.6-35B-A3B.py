...
the real checkpoint safetensors files, using the vendored reference
modeling on CPU
torch bf16.
...
What is generated (under tests/fixtures/layers/Qwen3.6-35B-A3B-layer-0/):

  gdn-Qwen3.6-35B-A3B-00.safetensor (+ .metadata.json.zip)
  ...
  gdn-Qwen3.6-35B-A3B-01.safetensor (+ .metadata.json.zip)
...
Run (twice; cmp proves byte determinism):
  cd <worktree root> && .venv/bin/python \
    workspace/transformers/tests/testgen/gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B.py
...
# bytes. QWEN36_VENDORED_SRC must point at its src directory. There is no default location.
VENDORED_SRC = os.environ.get("QWEN36_VENDORED_SRC")
if not VENDORED_SRC or not os.path.isdir(VENDORED_SRC):
    raise SystemExit(
        "[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] set QWEN36_VENDORED_SRC to the src directory of the pinned transformers "
...
FIXTURE_DIR = os.path.join(
    GRANDPARENT_DIR, "fixtures", "layers", "Qwen3.6-35B-A3B-layer-0"
)
...
def vm_page_size() -> int:
    ...
    raise SystemExit("[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] vm_stat gave no page size line")
...
def free_bytes() -> int:
    ...
    for line in out.stdout.splitlines():
        if line.startswith("Pages free:"):
            ...
            return pages * page
    raise SystemExit("[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] vm_stat gave no 'Pages free' line")
...
def check_ram() -> None:
    ...
    if free < MIN_FREE_BYTES:
        raise SystemExit(
            f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] free memory {free / 1024 ** 3:.1f} GiB below "
    ...
    if stray:
        raise SystemExit(
            f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] other python/torch processes hold RAM: {stray}; "
...
def check_vendored_sha() -> str:
    ...
    if sha != VENDORED_SHA:
        raise SystemExit(
            f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] vendored tree sha {sha} differs from "
...
def generate_gdn_prefill_fixture(block: Qwen3_5MoeGatedDeltaNet, cfg: Qwen3_5MoeTextConfig) -> None:
    ...
    assert torch.equal(module_output, chunk_replay["output"]), (
        "[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] chunked replay diverged from the module forward")
    ...
    assert output_diff < BLOCK_BAR, (
        f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] recurrent-vs-chunked output diff outside "
        f"(0, {BLOCK_BAR}): {output_diff}")
    assert ssm_diff <= ssm_cap(seq_replay["ssm_state"][0]), (
        f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] recurrent-vs-chunked SSM diff outside the "
    ...
    print(f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] prefill T={PREFILL_SEQ}: "
...
def generate_multichunk_fixture(block: Qwen3_5MoeGatedDeltaNet, cfg: Qwen3_5MoeTextConfig) -> None:
    ...
    assert core_diff < CORE_BAR, (
        f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] recurrent-vs-chunked core diff outside "
    ...
    print(f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] multichunk T={MULTICHUNK_SEQ}: "
          f"output diff {output_diff:.3e}, core diff {core_diff:.3e}, "
          f"ssm diff {ssm_diff:.3e}")
...
def main() -> None:
    ...
    print(f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] torch {torch.__version__}, vendored sha {sha[:12]}")
    print(f"[gen_bf16_02_gdn_fixtures_Qwen3.6-35B-A3B] wrote {FIXTURE_DIR}")
