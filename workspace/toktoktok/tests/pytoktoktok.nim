...
#
# This module exports the toktoktok pipeline machine to Python using nimpy.
# Compile command:
...
## Toktoktok Python extension module (nimpy binding), the pipeline machine export.
import std/tables
import std/strutils
...
proc normRegexEsc(s: string): string =
  ...
  ##   regex character classes where the Nim pattern constants carry
  ##   the backslash-escaped two-character sequences, regex-equivalent,
  ##   so matching runs on the normalized form.
  result = s
  for pair in [("\r", "\\r"), ("\n", "\\n"), ("\t", "\\t")]:
...
proc familyFromRegexp(regexp: string): scan.Family =
  ...
  ## the HF-to-tiktoken conversion derives from a checkpoint json
  ## pre_tokenizer:
  ##   single-regex families match their chain pattern
  ## (normalized), the step-3.5-flash chain matches its three steps
  ## joined in chain order. One alias row:
  ##   the ling checkpoint json
  ## spells the case-insensitive suffix alternation
  ## '(?i:[sdmt]|ll|ve|re), where the Nim constant spells
  ## (?i:'s|'t|'re|'ve|'m|'ll|'d), the same alternation. Loud failure
  ## on unknown shapes.
...
proc mkPipeline(codec: TiktokenCodec, fam: scan.Family,
  ...
  if withSpecials:
    ...
    for token, id in codec.specials:
      pats.add token
      ids.add id
    result = TokPipeline.init(codec.ranks, pats, ids, fam)
  else:
    result = TokPipeline.init(codec.ranks, @[], @[], fam)

proc hfFamily(path: string): scan.Family =
  let hf = deserializeHfTokenizer(readFile(path))
  familyFromRegexp(convertHfToTiktoken(hf).pattern)
...
proc load_tokenizer_hf*(path: string): PipelineRef {.exportpy.} =
  ...
  result.pipe = mkPipeline(codec, result.fam, true)
...
proc load_tokenizer_hf_ordinary*(path: string): PipelineRef {.exportpy.} =
  ...
  result.codec = codec
  result.fam = hfFamily(path)
  result.pipe = mkPipeline(codec, result.fam, false)
...
proc ordinaryPipeline(self: PipelineRef): TokPipeline =
  ...
  if self.plain.isNil:
    self.plain = TokPipeline.init(self.codec.ranks, @[], @[], self.fam)
  self.plain

proc pullBatch(p: TokPipeline, cap: int): seq[int] =
  ## One bounded consumption step, at most `cap` ids taken per call.
  ## The machine keeps its position state (in-flight decision carry)
  ## for the next pull, repeat until drained() reports true.
...
proc drainAll(p: TokPipeline): seq[int] =
  ## Whole-stream drain as a bounded-consumption loop, one-shot
  ...
  const PullCap = 4096
  while not p.drained():
    result.add pullBatch(p, PullCap)
