...
## 1. chain fixture frames (exaone, step-3.5-flash, gemma-4), every row
##    of the HF-recorded frames must be reproduced byte-identically,
## 2. unit rows per chain feature:
##    digit grouping \p{N}{1,3}, the CJK-script split step,
##    the GPT-2-style letter/combining-mark pattern, the Gemma-4
##    MergedWithPrevious space splitter (expectations hand-recorded from HF tests),
## 3. chain-vs-flat-join divergence rows:
##    joining a chain's patterns into one alternation
##    (the convertHfToTiktoken flaw, the operator-pending issue-22 class recorded on the toktoktok side)
##    must diverge exactly on the documented rows and nowhere else,
...
## 7. machine stream rows:
##    empty input yields nothing, one-pass stream equals the collected piece list.
...
proc runTests*() =
  ...
  for entry in [
    ...
    for row in readChainFrame(family).items:
      let got = collectPieces(fam, row.text)
      if got != row.pieces:
        inc frameFails
  ...
  block:
    ...
    for c in cases:
      let got = collectPieces(c.f, c.text)
  ...
  block:
    ...
    for c in cases:
      let got = collectPieces(famStep35Flash, c.text)
  ...
  block:
    ...
    let got = collectPieces(famExaone, "e\u0301x")
  ...
  block:
    ...
    for c in cases:
      let got = collectPieces(famGemma4, c.text)
      check "gemma MergedWithPrevious " & c.text.escape,
        piecesEq(got, c.want), firstDiff(got, c.want)

  #
  # 3. chain-vs-flat-join divergence rows (operator-pending issue-22 class):
  ...
  block:
    ...
    var sameCount = 0
    for row in rows.items:
      let chain = collectPieces(famStep35Flash, row.text)
      ...
      if chain == flat:
        ...
        inc divCount
        echo "documented divergence row (issue-22 flat-join class): ",
          row.text.escape
  ...
  block:
    ...
    for d in c.items:
      if d.specialId >= 0:
        ...
        let standalone = collectPieces(famStep35Flash, region)
    ...
    for d in c2.items:
      if d.specialId >= 0:
        ...
        for s in collectPieces(famStep35Flash, text[lo ..< hi]):
          cov += s[1] - s[0]
  ...
  block:
    ...
    var r = initPreTokenizer(famStep35Flash, CjkText)
    var warmPieces = 0
    for piece in r.items:
      inc warmPieces
    ...
    for i in 0 ..< 2000:
      r.reset(famStep35Flash, CjkText)
      for piece in r.items:
        discard piece
  ...
  block:
    for f in Family:
      discard familySteps(f)
    let st = allFamilyStats()
    for f in Family:
      let s = st[f]
  ...
  #
  block:
    var r = initPreTokenizer(famStep35Flash, "")
    var count = 0
    for piece in r.items:
      inc count
    check "empty input yields nothing", count == 0
    let text = "a1234567b中文字c"
    let want = collectPieces(famStep35Flash, text)
    var r2 = initPreTokenizer(famStep35Flash, text)
    var got: seq[(int, int)] = @[]
