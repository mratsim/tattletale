...
## | # | check                                                                                                                                                                                      |
## | --- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
## | 1 | remap table == an independently computed GPT-2 bytes_to_unicode table, and every mapped codepoint round-trips through `unmap`, cross-checked against the deserializer's GPT-2 byte decoder |
## | 2 | corpus streams: `bytesToUnicode` output unmaps back to the corpus text byte-identically (the remap is a bijection over bytes)                                                              |
## | 3 | prefix-space position: add_prefix_space=true prepends one space unless the input already opens with one                                                                                    |
## | 4 | the remap string is the full mapped alphabet: nonempty for any nonempty input and it unmaps back byte-identically                                                                          |
