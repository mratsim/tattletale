#!/usr/bin/env python3
"""Issue #22 special-pretokenization failing vectors
as first-class parity-corpus rows.

Fold the concrete failing vectors of issue #22
(mratsim/tattletale special-pretokenization) into the toktoktok parity corpus:

the raw corpus texts (sanguozhi, shakespeare, verne, sqlite prefixes, the same max_chars cuts as the toktoktok harnesses use) encoded
by the reference engines for the three failing families:

- exaone (K-EXAONE-236B-A23B, HF tokenizers library)
- step-3.5-flash (Step-3.5-Flash, HF tokenizers library)
- kimik2.5 (Kimi-K2.5, tiktoken library over the KIMI_K25 pattern)

One output frame per family, zstd level 19 + content size + checksum
(zstd_frame conventions), rows in the shared
{name, text, tokenIds, tokenizer} format.

Byte-determinism rule:
  run this script TWICE, sha256 both outputs,
commit only when the hashes match run-to-run.

Usage (repo root or anywhere, paths are absolute-inside-script):
