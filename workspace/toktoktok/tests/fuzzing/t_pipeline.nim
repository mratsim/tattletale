...
proc runTests*() =
  ...
  block:
    ...
    var pChunk = pipeline.TokPipeline.init(codec.ranks, pats, ids,
  ...
  block:
    ...
    var p = pipeline.TokPipeline.init(codec.ranks, pats, ids, scan.Family.famCl100k)
    pipeline.resetText(p, text)
    var got: seq[int] = @[]
    var taken = 0
    for id in p.items:
    ...
    block:
      var pMid = pipeline.TokPipeline.init(codec.ranks, @["<a>"], @[999],
        scan.Family.famCl100k)
    ...
    var p2 = pipeline.TokPipeline.init(codec.ranks, @[], @[], scan.Family.famCl100k)
    ...
    var fedOk = true
    try:
      var p3 = pipeline.TokPipeline.init(codec.ranks, @[], @[], scan.Family.famCl100k)
