...
## Kimik2.5-ranks speed board, full-pipeline wall over the four
## corpus prefixes (verne-5k, shakespeare-10k, sanguozhi-10k CJK, sqlite-50k), Nim-native medians.
##
## Reference comparisons come from running this bench
## and the cross-check suites, not from recorded results.
##
## Rows per corpus:
## one warm-up rep (engine tables, pre-tokenizer buffers and the rank table reach steady state)
## dropped from the timing, then `reps` timed reps, the median wall
## and MB/s per row. Stage-split rows mark where the wall lives:
## - `scan-only`, the pre-tokenization machine's piece stream alone
##   (the pattern-split scan where the step carries one, the frontier-engine scan otherwise).
## - `scan-only-frontier`, the same machine
##   forced down the frontier-engine scan (the A/B row against the pattern-split scan).
## - `bpe-only`, the ordinary PQ-merge tail (whole-piece hit first, else the bytePairEncodePQ merge core).
## - `bpe-only-bt`, the ordinary backtracking tail (whole-piece hit first, else the bt core).
## Every timed rep's id stream is asserted equal to the reference
## stream built untimed per corpus:
## - whole-piece rank hit first, else the bytePairEncodePQ merge core
##   per pre-tokenized piece.
## - the reference is built by the priority-queue reference core
##   (the test-side engine, no served path),
##   independent of the pipeline's served backtracking tail.
## Parity belongs to the test suites, this assert is the bench's loud sanity check.
##
...
proc bpeOnlyOrd(engine: BpeEngine, ranks: Table[seq[byte], int],
    text: string): seq[int] =
  ## Ordinary BPE tail over the family's pre-tokenized pieces:
  ## - whole-piece rank hit first, else the bytePairEncodePQ merge core per piece.
  ## - the PQ merge core is the test-side reference core, the wired
  ##   pipeline serves the backtracking core instead.
  ##
  ...
  for (lo, hi) in r.items:
    ...
    if piece in ranks:
      ...
      bytePairEncodePQ(engine, m, data, lo, hi, scratch)
...
proc bpeOnlyBt(engine: BpeEngine, bt: var BacktrackBuf,
  ...
  for (lo, hi) in pieces:
    buf.setLen(0)
    encodeSegment(engine, bt, data, lo, hi, buf)
    for x in buf.items:
      result.add x
...
proc main() =
  ...
  for (name, file, prefixBytes) in Corpora.items:
    ...
    let reference = bpeOnlyOrd(refEngine, codec.ranks, text)
    var walls: seq[float] = @[]
    var ids0: seq[int] = @[]
    ...
    # (PQ merge core over the pre-computed pieces, ids re-asserted)
    ...
    for k in 0 ..< reps + 1:
      let w0 = nowMs()
      bpeIds = bpeOnlyOrd(refEngine, codec.ranks, text)
      let w1 = nowMs()
    ...
    echo "[bench-kimik2.5] ", name, " bpe-only(PQ) medianMs=",
    ...
    for k in 0 ..< reps + 1:
      ...
      for (lo, hi) in pieces.items:
        scratch.setLen(0)
        encodeSegment(refEngine, bt, data, lo, hi, scratch)
        for x in scratch.items:
          idsBt.add x
      let w1 = nowMs()
