...
## - whole-piece hits, a pre-tokenized piece whose full byte string
##   is a vocabulary key.
## - pair-rank lookups, byte-slice views into a segment.
## - keys never live in the hot path, the trie is a flat
##   BASE/CHECK/VALS triple (Aoe's double-array scheme, the same BASE+CHECK core as the special-scan automaton).
## - a lookup walks the key bytes in place, 2 loads per byte.
##
## Construction:
...
##   key (total work O(total key bytes)), and children lists come out
##   naturally in byte order.
...
## - `chk[i] == 0` means slot i is free.
## - `chk[i] == p >= 1` means slot i holds the child of node p at byte
##   i - base[p], check values are parent indices.
## - `chk[i] < 0` means slot i is reserved (slot 0, the root slot).
## - `base[p] == 0` means node p is a leaf (no children).
## - `vals[s] >= 0` means a key ends at slot s, with rank vals[s].
## - `vals[s] == -1` means the path to slot s is a strict prefix only.
##
...
proc placeChildren(nodes: openArray[TrieNode], buildNode, slot: int32,
    base: var seq[int32], chk: var seq[int32], vals: var seq[int32],
    cursor: var int): int32 =
...
proc assignDfs(nodes: openArray[TrieNode]): VocabTrie =
  ...
  while stack.len > 0:
    ...
    let b = placeChildren(nodes, buildNode, slot, base, chk, vals, cursor)
...
proc longestMatch*(t: VocabTrie, data: openArray[byte], lo, hi: int): int =
  ...
  ## RankNotFound when no key matches. The walk tracks the deepest
  ## key end seen, one trie step per input byte, so a match's length
  ## is read off the winning key without a rescan.
  ## Expected input:
...
proc containsKey*(t: VocabTrie, data: openArray[byte],
    lo, hi: int): bool {.inline.} =
  ## True when data[lo ..< hi] is a vocabulary key.
  t.lookup(data, lo, hi) >= 0
...
##
## - TokenArena, all vocab keys concatenated, indexed by dense id
##   (dense ids = rank-ascending order, byte-ordered tiebreak for duplicate ranks, so children ids sort below parent ids).
## - PairIndex, the exact-match map over every valid merge pair,
##   key = (left dense id, right dense id) packed,
##   value = the merged dense id:
##   - multiply-shift hashed stride with the factor fixed at load over
##     frozen keys, cumulative-offset bucket arrays, no probing.
##   - the worst bucket is verifiable at load and asserted there, the whole guarantee of a static structure over a frozen vocabulary.
##   - this replaces the pair-span trie walk plus its memo cache
##     (bench-confirmed 11.4 vs 24.6 ns mixed on the real pair workload, lctrie_bench D2, whose buildPairIndex is cribbed here).
## - SplitTable, dense id -> (left id, right id) flat arrays plus
##   nextPrefixMatch (longest proper-prefix token):
##   - derivation per the rust-gems recipe, the byte_pair_encoding.rs
##     from_dictionary tail, greedy first-valid-split over the prefix chain,
##     checked by is_valid_token_pair against the partially built tables.
##   - the limit-loop rule, a pair hit with combined below limit
##     means the adjacency is frozen-invalid (byte_pair_encoding.rs:97-125).
##   - tokens with no valid split carry the self-split (id, id).
## - selfCheck, every vocab token re-encodes to itself through
##   the built tables (priority-queue rank merge over PairIndex plus the probe_selfcheck.py port):
##   - the check hard-asserts zero failures when the rank table is
##     a dense permutation of 0 ..< n, the tiktoken shape every served family has.
##   - synthetic tables with duplicated or non-dense ranks only get the receipt.
##
...
proc pairHash(a, b: uint32, factor: uint64, bits: int): int {.inline.} =
  ## Multiply-shift hashed stride over the packed pair key, cribbed from the probed D2 lctrie_bench implementation.
...
proc isValidPairDerivation(pairs: var Table[(uint32, uint32), uint32],
  ...
  ## - otherwise the limit-loop walks the split chain, polarity per
  ##   byte_pair_encoding.rs:97-125, never inverted.
  var token1 = t1
  var token2 = t2
...
proc selfCheck(t: MergeTables): tuple[unreachable, singleIdWrong: int] =
  ...
  ## the probe_selfcheck.py port (priority-queue rank merge, rank source = PairIndex id-pair lookups),
  ...
  ## Vocabularies converted from HF checkpoints carry tokens
  ## unreachable by merging (special-like markers whose byte pieces merge into other tokens first):
  ## - a nonzero unreachable count is a data property there, checked
  ##   by the id-parity suites.
  let n = t.starts.len
...
## - the priority-queue reference implementation lives in the fuzz
##   tree (tests/fuzzing/pq_bpe_reference.nim), the cross-check suites
##   check the two byte-identical on every family over corpora, fuzz
##   and adversarial rows.
## - the algorithm below ports the rust-gems crate's no-heap lazy-DP backtracking encoder.
##
## Algorithm:
## - place the longest vocabulary match at the cursor and walk forward.
## - on a frozen-invalid adjacency, walk the failed token's
##   proper-prefix chain (progressively shorter matches).
## - when the chain exhausts, the boundary is a dead end, mark it
##   dead in the bitfield, pop the last placed token and retry it
##   from its start, and the popped token becomes the next
##   candidate while the dead bit at its old end position forces
##   the retry down the prefix chain.
## - positions whose every continuation is dead are never re-entered,
##   so the walk visits each reachable DP state once.
## - all vocabulary tokens matching the text at one position form
##   exactly the longest match's prefix chain, so the chain walk
##   covers the full candidate set.
##
## Contract with the structures (MergeTables):
## - ids are dense ids end to end, the output walk maps them back
##   to ranks through denseToRank.
## - isValidTokenPair is the frozen-adjacency predicate, polarity
##   of the derivation referee (merge_tables.nim):
##   - a PairIndex HIT with the merged id below the split-chain limit
##     means the adjacency is INVALID (the merge process would have merged across it).
##   - validity is the absence of such a hit along the limit-bounded
##     split_table chain walk, never "radix hit = valid".
## - dead-end semantics, a segment whose every continuation dead-ends
##   emits the tokens placed so far (possibly none), the reference
##   behavior. The PQ reference instead raises BpeError on pieces it
##   cannot encode. The two can co-diverge only on vocabularies
##   missing single bytes or carrying unreachable merged tokens, every
##   served family covers all 256 single bytes and self-checks clean,
##   and the cross-check suite checks bt == PQ on every family
##   over corpora, fuzz and adversarial rows.
## - the 1-byte double-add quirk of the naive merge core is NOT
##   replicated here, the quirk is unreachable through the ordinary
##   path whenever every single byte is a vocabulary token, since
##   the whole-piece branch emits it first.
##
## No-copy discipline (the 287-us-per-call trap):
...
proc isValidTokenPair*(e: BpeEngine, t1, t2: uint32): bool =
  ...
  ## Polarity contract, faithful to the derivation-time predicate
  ## in the derivation section (merge_tables.nim, same polarity, complete tables).
  e.ensureBuilt()
  var token1 = t1
  var token2 = t2
  var limit = uint32.high
...
proc bytePairEncodeBt*(e: BpeEngine, bt: var BacktrackBuf,
    data: openArray[byte], lo, hi: int, dst: var seq[int]) =
  ## Backtracking merge core over the byte slice [lo, hi):
  ## - the module-header lazy-DP walk, ids stream dst-first in ranks
  ##   (denseToRank-mapped at the output walk).
  ## - a segment the walk cannot fully cover emits the tokens placed
  ##   so far (dead-end semantics, unreachable for vocabularies covering all 256 single bytes, checked by the cross-check suite).
  ## Output:
...
proc encodeSegment*(e: BpeEngine, bt: var BacktrackBuf,
    data: openArray[byte], lo, hi: int, dst: var seq[int]) =
  ...
  if whole >= 0:
    ...
    bytePairEncodeBt(e, bt, data, lo, hi, dst)
