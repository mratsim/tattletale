...
## Regex pattern engine for the tokenizer front-end (parser, NFA compiler, leftmost-first matcher, frontier-DFA compile).
##
## Flow, one Thompson NFA program drives two engines:
##
##   pattern ──compilePattern──▶ Thompson NFA program
##     │
##     ├─▶ NFA simulation (attempt)          leftmost-first, parity reference
##     └─▶ frontier-DFA compile (default)    memoized table walk
##         determinization cap hit ──▶ permanent NFA fallback
##
## Matching is leftmost-first, not a powerset DFA:
## - a powerset DFA is a longest-match machine.
## - a PCRE2 backtracking-priority scan returns the first match in pattern-preference order, not the longest extent.
## - the two extents differ when alternatives overlap (the trailing whitespace group `\s++$|\s+(?!\S)|\s`, or overlapping letter alternatives).
##
## Engine shape (RE2 / Rust-regex leftmost-first Thompson simulation):
## - threads carry the pattern preference order, expanded depth-first per slot.
## - the cut rule makes an acceptance kill every lower-priority thread.
## - scan cost:
##
##   | condition                                            | cost                                                        |
##   | ---------------------------------------------------- | ----------------------------------------------------------- |
##   | dense matches                                        | O(len x program)                                            |
##   | an attempt inside a long backtracking-quantifier run | O(remaining run)                                            |
##   | adversarial single-rune-class runs of length L       | O(L^2) overall, beyond the parity corpora (fuzz suite rows) |
##
## Deterministic compile (frontier DFA):
## - the default scan driver memoizes the simulation's ordered frontiers and per-symbol transition tables.
## - a plain powerset DFA cannot carry the leftmost-first extents.
## - the NFA simulation stays the parity reference and the overflow fallback when the determinization cap is hit.
##
## Supported constructs:
## - alternation, concatenation, non-capturing groups, caseless groups (?i:...).
## - single-codepoint negative lookaheads `(?!\S)` and `(?!\p{Script=Han})`, compiled as a complement class plus an end-of-window test.
## - the $ assertion with PCRE2 semantics (end of subject or before a final LF).
##
## Quantifiers and classes:
## - character classes with \p{...} members and complements.
## - the quantifiers * + ? {m,n}, each greedy, the possessive suffix (+) accepted and compiled greedy.
## - possessive-as-greedy holds when the continuation is one of these kinds:
##
##   | continuation                                        | note                                                                  |
##   | --------------------------------------------------- | --------------------------------------------------------------------- |
##   | absent                                              | end of alternative, nothing follows to back off into                  |
##   | the $ assertion                                     | its before-final-LF position can never lie strictly inside a `\s` run |
##   | a class provably disjoint from the quantified class | `[^\r\n\p{L}\p{N}]?+` before `\p{L}++`                                |
##
## - greedy and possessive therefore admit identical match extents, proven by the parity suites against the PCRE2 reference.
##
## NOTEMPTY and Unicode:
## - a zero-width total acceptance is discarded, mirroring PCRE2 NOTEMPTY.
## - Unicode classes come from the regex_unicode_tables module, generated against the PCRE2 UCD.
## - Rust-regex callers resolve `\s` through the White_Space definition (the whitespaceIsRust compile flag).
##
## - U+180E sits in the PCRE2 UCP `\s` table but not in White_Space (dropped in Unicode 6.3), a known divergence, so parity fuzz rows carry it.
...
const
  ...
  RustSpaceRanges* = [
    # Rust regex crate `\s` == Unicode White_Space (the crate documents `\s` as `\p{White_Space}`).
    # Distinct from the PCRE2 UCP table, which still carries U+180E.
...
type
  ...
  InstrKind* = enum
    ...
    iLookNeg  # zero-width, pass when no class codepoint sits at the cursor
    iDollar   # zero-width, end of window or before a final LF
    iMatch    # accept which ends the attempt

  Instr* = object
    ## One NFA instruction (class consume, split, negative lookahead, $ assertion or accept).
    case kind*: InstrKind
    of iChar, iLookNeg:
      cls*: ClassId
      next*: int32
  ...
  DfaState = object
    ## One frontier state of the deterministic compile:
    ## the ordered live-thread list of the simulation at some step,
    ## plus the memoized transition table and end-of-window verdict.
    threads: seq[int32]
  ...
  DfaEngine = ref object
    ## Deterministic frontier machine for one pattern:
    ## - states are the ordered live-thread frontiers (the simulation's clist contents).
    ## - transitions are the simulation's closure plus consume step,
    ##   memoized per symbol context, a match is a table walk reproducing
    ##   attempt() bit for bit without re-running epsilon closures at steady state.
    ## - construction is lazy and memoized, first use compiles each
    ##   reachable state once (single-threaded per pattern like the NFA scratch buffers).
    states: seq[DfaState]
    ...
    work: seq[int32]
    frontier: seq[int32]
    # Symbol table compiled once at engine attach.
    # The membership vector is constant between class range boundaries,
    # so one table read (ASCII plane) or one binary search replaces
    # the per-class membership tests.
    symAscii: array[256, uint64]
    symBounds: seq[uint32]
    symVecs: seq[uint64]

  PatternCompileError* = object of ValueError
    ## Raised for unsupported constructs and malformed patterns.

  # One compiled pattern (NFA program plus class table).
  CompiledPattern* = ref object
    ## Scratch buffers make attempts allocation-free once warm.
    ## They are shared per compiled pattern (single-threaded stage use, no nesting).
    prog*: seq[Instr]
    ...
    stack*: seq[int32]
    name*: string
    # First-codepoint prefilter, the union of the iChar classes
    # reachable through the start epsilon closure.
    # Lookaheads and assertions count as transparent (an over-approximation).
    #
    # A codepoint outside it can never be consumed by any thread
    # spawned at the start, so no non-empty match can begin there
    # (attempt() would fail anyway).
    # Zero-width acceptances are dropped by the NOTEMPTY rule and contribute nothing.
    ...
    # transition key width (the NFA simulation then stays the engine).
    dfa*: DfaEngine
...
# ───────────────────────────────────────────────────────────────────────
# Codepoint classes and finished-range helpers
# ───────────────────────────────────────────────────────────────────────

proc classAdd*(c: var CpRanges, lo, hi: uint32) =
  ## Appends one inclusive range, unsorted (classFinish merges later).
  c.rs.add (lo, hi)
...
proc classUnion*(a, b: CpRanges): CpRanges =
  ## Union of two finished ranges, re-finished.
  result = a
  for r in b.rs.items:
    result.rs.add r
...
proc classContains*(c: CpRanges, cp: uint32): bool {.inline.} =
  ## Binary-search membership over the finished ranges.
  var lo = 0
  var hi = c.rs.len
  while lo < hi:
    let mid = (lo + hi) div 2
...
proc buildAsciiBits(c: CpRanges): array[4, uint64] =
  ...
  ## only. Bit cp is set iff classContains(c, cp.uint32) holds for cp < 256.
  for r in c.rs.items:
    if r.lo <= 255'u32:
...
proc parseGroup(p: var Parser): Node =
  ## Entry cursor sits on the group-open character.
  inc p.pos
  if p.peek() == '?':
    inc p.pos
    case p.peek()
    ...
    of '!':
      inc p.pos
      # single-codepoint negative lookahead, compiled as a complement
      # class plus an end-of-window test, fully regular
      var negCls: CpRanges
      if p.peek() == '[':
...
proc parseQuant(p: var Parser, atom: Node): Node =
  ## Reads an optional quantifier suffix:
  ## greedy by default, the possessive suffix + accepted and compiled
  ## greedy (see the module doc), lazy ? rejected with a compile error.
  var minRep = 1
  var maxRep = 1
  case p.peek()
...
# ───────────────────────────────────────────────────────────────────────
# NFA compiler (Thompson lowering)
# ───────────────────────────────────────────────────────────────────────

type
  HoleTarget = tuple[instr: int, field: int]
    ## field 0 patches the next of iChar / iLookNeg / iDollar,
    ## field 1 the split a branch, field 2 the split b branch.
...
proc chainFrag(acc: var Frag, prog: var seq[Instr], f: Frag) =
  ## Concatenates fragment f after acc (the acc entry stays, its holes now point at f).
  if acc.entry == -1:
    acc = f
    return
...
proc lowerStar(p: var Parser, prog: var seq[Instr],
  ...
  ## Greedy star, a split with the body preferred, the body loops back.
  ...
  of nAlt:
    ...
    if frags.len == 1:
      ...
      for f in frags.items:
        holes.add f.holes
      # dispatch chain, splits prefer each branch in declaration order,
      # the last branch is entered by the final split fall-through
...
proc compilePattern*(pattern: string, whitespaceIsRust = false,
  ...
  # flag bit in 64 bits, so wider patterns keep the NFA simulation
  # (the served patterns stay far below the width).
  if result.classes.len <= 62:
    result.dfa = DfaEngine(visited: newSeq[int64](result.prog.len))
    dfaBuildSymbols(result, result.dfa)

# ───────────────────────────────────────────────────────────────────────
# Leftmost-first matcher (the NFA simulation attempt)
# ───────────────────────────────────────────────────────────────────────

proc decodeCp*(input: string, pos, hi: int): tuple[cp: uint32, width: int] =
  ## Decodes one UTF-8 codepoint at pos.
  ## - valid sequences yield (codepoint, byte width).
  ## - anything else yields the sentinel codepoint 0xFFFFFFFF
  ##   with width 1, a codepoint that matches no class.
...
proc classHas*(p: CompiledPattern, cls: ClassId, cp: uint32): bool {.inline.} =
  ## Bitmap-first class membership for the matcher hot path.
  ## Returns the same verdict as classContains, bitmap-backed below
  ## 256 and binary-searched above.
  ## - the bitmap is derived at compile time from the same finished
  ##   ranges classContains binary-searches.
  ## - the two predicates agree bit for bit on the whole ASCII plane,
  ##   the fuzz suite checks the explicit equivalence.
  if cp < 256:
    let idx = int(cp)
...
proc attempt(p: CompiledPattern, input: string, winLo, winHi, s: int): int =
  ...
  ## (a codepoint boundary inside [winLo, winHi)).
  ## Returns the match end, or -1 when nothing non-empty matches from s.
  ...
  p.clist.add p.start
  while true:
    # Single-iChar-thread fast path. With exactly one live waiter whose
    # instruction consumes, no epsilon closure can fire and no accept
    # sits upstream, so the generic slot machinery below rebuilds
    # exactly this step (a one-element nlist, the byte filter, a plain advance).
    # Both exits replicate the generic loop.
    # A waiter moving beyond the window breaks with the recorded extent
    # (an empty nlist). A class miss breaks the same way, a following
    # iMatch or split re-enters the generic path.
    ...
    for i in 0 ..< p.clist.len:
      ...
      while p.stack.len > 0:
        ...
        if cut:
          break
    # an acceptance kills every lower-priority thread (the remaining slots and the rest of this slot subtree) but the attempt keeps
    # running. Surviving higher-priority threads still accept later
    # and override the recorded extent (greedy loops keep extending)
    if p.nlist.len == 0:
      break
...
# ───────────────────────────────────────────────────────────────────────
# Frontier DFA compile of the leftmost-first simulation
# ───────────────────────────────────────────────────────────────────────
#
# Deterministic-compile flow:
#
#   pattern ──compilePattern──▶ NFA program
#        │
#        ├─▶ attempt(): the leftmost-first NFA simulation, the parity
#        │     reference and the permanent overflow fallback
#        │
#        └─▶ dfaBuildSymbols(): the symbol contexts, one membership
#              │  vector per codepoint over the interned class table plus
#              │  the dollar-window flag
#              ▼
#        ├─▶ frontier-DFA compile: states are the simulation's ordered
#              │  live-thread frontiers, transitions are the closure-plus-
#              │  consume step for one symbol context, memoized
#              ▼
#        └─▶ steady-state matching: a memoized per-symbol table walk
#              │  that records the extent and applies the cut rule
#
# Why not a powerset DFA:
# - a powerset DFA is a longest-match machine.
# - the NFA simulation is leftmost-first, its extent differs from the longest
#   extent when alternatives overlap, as in the GPT-2 trailing whitespace
#   group or the paired letter alternatives (o200k, Kimi, and Moonlight patterns).
# - this compile preserves leftmost-first semantics by determinizing the simulation's ordered frontiers instead of plain instruction sets.
#
# State shape and steps:
# - a DFA state is the exact ordered live-thread list the simulation
#   carries (slot preference order, first-occurrence dedup), a transition
#   is the closure-plus-consume step for one symbol context, memoized
# - matching is a table walk, epsilon closures never re-run at steady state
# - every recorded extent, cut, and truncation happens at the same step
#   as in the NFA simulation, which makes the two paths step-for-step
#   replicas (the fuzz suite checks byte-identical segmentation)
#
# Symbol contexts:
# - one context = the membership vector of the current codepoint over
#   the interned class table plus the dollar-window flag (the codepoint is a LF that ends the window)
# - lookahead folds into the context, and the end-of-window context
#   is handled by a per-state verdict instead of a table entry
#   (no codepoint is consumed there)
# - the transition function is a pure function of (frontier, context),
#   so memoization is sound

const
  MaxDfaStates = 50_000
  ## Lazy determinization cap per pattern.
  ## - the served patterns compile to modest frontier counts on ordinary text, so the cap bounds pathological inputs only
  ## - hitting it permanently reverts that pattern to the NFA
  ##   simulation path (same output, the NFA path is the step-for-step replica)

proc dfaBuildSymbols(p: CompiledPattern, e: DfaEngine) =
  ## Compiles the symbol table once at engine attach.
  ## - sorted class range boundaries partition the codepoint space
  ##   into cells with a constant membership vector, one per cell
  ## - a linear sweep over per-class range events fills the vectors,
  ##   no per-cell searches, vectors derived from the same finished
  ##   ranges that yielded the classHas bitmaps
  ## - the plane above the last boundary and the invalid-byte sentinel
  ##   read as the all-zero vector, exactly like classContains
  ...
  for c in 0 ..< p.classes.len:
    doAssert active[c] == 0
  # sentinel boundary, an extra all-zero cell above the last event
  # keeps every codepoint up to MaxCodepoint inside a searched cell,
  # and the invalid-byte sentinel plane above reads all-zero exactly
  # like classContains does
...
proc dfaStep(p: CompiledPattern, e: DfaEngine, threads: openArray[int32],
  ...
  ## One deterministic replication of the simulation step.
  ## - epsilon closure slot by slot in frontier order with first-visit
  ##   dedup and the cut rule at an acceptance
  ## - then the consume stage (filter by class membership, map to continuations, first-occurrence dedup)
  ## - writes the next frontier into e.frontier, the scratch buffers
  ##   are engine members reused per step
  ## Returns whether an acceptance fired (the caller records the extent at its own position).
  e.vgen += 1
  let g = e.vgen
  e.frontier.setLen(0)
  var cut = false
...
proc dfaIntern(p: CompiledPattern, e: DfaEngine): int32 =
  ## Interns the frontier just computed into the state table.
  ## Returns the new state id, or -1 for a dead frontier (empty) or a determinization cap hit.
  ## - the stored copy never aliases the engine scratch buffers
  ##   (they are reused and mutated by the next step)
  if e.frontier.len == 0:
    return -1
...
proc dfaStartState(p: CompiledPattern, e: DfaEngine, bits: uint64,
    dollarOk: bool): int32 =
  ## Initial frontier for one anchored attempt.
  ## - the closure of the start thread at the attempt position, accepts
  ##   inert (NOTEMPTY drops them, no cut fires at the attempt position), then the consume stage
  ## - memoized per symbol context
  var key = bits shl 1
  if dollarOk:
    key = key or 1
...
proc dfaStepState(p: CompiledPattern, e: DfaEngine, sid: int32,
  ...
  ## Memoized transition of one state on one symbol context.
  ## Returns the packed transition entry:
  ## - (state id + 1) shifted left once with the acceptance bit in the low bit
  ## - a dead continuation packs to 0 or 1, every present entry stays nonnegative
  var key = bits shl 1
  if dollarOk:
    key = key or 1
...
proc dfaEofAccept(p: CompiledPattern, e: DfaEngine, sid: int32): bool =
  ## End-of-window verdict of one state, memoized.
  ## - the closure runs with every lookahead and the dollar assertion passing
  ##   (no consuming waiter is collected, the window is exhausted)
  ## - the verdict is whether an acceptance fired, nothing survives
  ##   the step, and the attempt ends exactly like the NFA loop
...
proc attemptDfa(p: CompiledPattern, input: string, winLo, winHi, s: int): int =
  ## One anchored leftmost-first attempt as a frontier walk over the deterministic compile.
  ## - initial frontier at s, then one memoized table step per consumed
  ##   codepoint with the recorded extent refreshed at every accepted step
  ## - extents grow with the position, so the last accepted step
  ##   carries the final extent, exactly like the cut-and-override
  ##   bookkeeping of the NFA simulation
  ## Returns -2 on a determinization cap hit, which flips the pattern
  ## to the permanent NFA fallback.
  doAssert s >= winLo and s <= winHi
  let e = p.dfa
  if s >= winHi:
    return -1
...
proc matchAt*(p: CompiledPattern, input: string, winLo, winHi, pos: int): int =
  ...
  ## Returns the match end (> pos), or -1 when nothing non-empty matches from pos.
  ##
  ## Engine selection:
  ##
  ##   ┌─ determinization live ──▶ frontier-DFA walk (attemptDfa)
  ##   └─ cap hit mid-call ──▶ latch permanent fallback ──▶ NFA simulation
  ##
  ## Window semantics match nextMatch:
  ## - `$` matches at winHi or before a final LF
  ## - no match extends past winHi, callers feeding slices of one
  ##   buffer get per-slice haystack semantics
  ## - frontier-DFA walk when the deterministic compile is live, the NFA
  ##   simulation otherwise, and a determinization cap hit mid-call latches
  ##   the permanent fallback and returns the NFA verdict
  ## Worked example:
  ## - pattern `\s+\s` over "  x", an attempt anchored at pos 0 returns 2
...
proc dfaMemoryEstimate*(p: CompiledPattern): int =
  ...
  ## a bound for receipts rather than an exact ledger.
  ## Returns the estimate, 0 when no engine is attached.
  ## - per state the thread list plus its memo table rows
  ##   (key + packed value + load-factor slack)
  ## - plus the interning index rows
  if p.dfa == nil:
    return 0
  for st in p.dfa.states.items:
...
proc nextMatchImpl(p: CompiledPattern, input: string, fromPos: int,
  ...
  ## simulation (parity reference implementation plus overflow fallback).
  ## Leftmost scan in PCRE2's find-all semantics:
  ## - the first position >= fromPos admitting a non-empty match wins,
  ##   with the pattern-preference extent
  ## - (-1, -1) when no position matches
  ## - a splitting caller emits the gap before each match as pieces,
  ##   the unmatched remainder becomes one trailing piece (the PCRE2 iterator stops at the first unmatched offset)
...
proc nextMatchNfa*(p: CompiledPattern, input: string, fromPos: int,
    winLo = 0, winHi = -1): tuple[start, stop: int] =
  ## NFA-simulation scan driver, the fuzz-suite reference
  ## implementation plus the overflow fallback path.
  ## - window semantics match nextMatch
  nextMatchImpl(p, input, fromPos, winLo, winHi, useDfa = false)

# ───────────────────────────────────────────────────────────────────────
# Scan machine over the scan drivers
# ───────────────────────────────────────────────────────────────────────

type
  RegexScanner* = object
    ## Match-position machine over one compiled pattern.
    ## - one items iteration yields the leftmost-first match spans in scan order
    ## - the cursor is the scan position between yields and advances before each yield, so a consumer that stops mid-stream resumes exactly there
    ## - no allocation, the machine state is the cursor plus the window bounds
    ##   the compiled pattern's scratch buffers are reused per attempt,
    ##   the input is held by reference
...
proc initRegexScanner*(pattern: CompiledPattern, input: string,
  ...
  ## [winLo, winHi) of it (winHi < 0 means input.len).
  ## Returns the bound scanner.
  ## - matches never extend past the window end
  ## Worked example, the pattern `\p{L}+` over "ab  cd" yields (0, 2) then (4, 6)
  let hi = if winHi < 0: input.len else: winHi
...
iterator items*(m: var RegexScanner): tuple[start, stop: int] =
  ## Iteration surface of the scanner.
  ## Returns the next non-empty leftmost-first match span per yield.
  ## - byte offsets are into the bound input, until no further position inside the window matches
  ## - post-drain iteration stays empty
