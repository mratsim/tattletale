...
## Regex engine semantics: parse/compile determinism, class bitmap
## equivalence, match semantics vs the bundled PCRE2 engine (direct
## oracle replica: UTF+UCP compile, NOTEMPTY + NO_UTF_CHECK match, the
## first position admitting a non-empty match wins and the scan stops
## at the first unmatched offset), leftmost-first pattern-preference
## extents, the two \s variants (U+180E included), DFA == NFA twin,
## and the compile-error model. Fixture patterns are the suite's own
## engine-representative regexes covering every supported construct.
...
type
  Pcre2Oracle = object
    ## Direct PCRE2 replica of the engine's reference scan: same compile
    ## flags (UTF+UCP), same match flags (NOTEMPTY + NO_UTF_CHECK),
    ## gap-before-match emission, scan stops at the first unmatched
    ## offset, remainder is one trailing piece.
    code: ptr Code
...
const
  ...
  # Together they exercise every supported construct: alternation,
  # concatenation, non-capturing + caseless groups, both lookahead
  # forms, the $ assertion, \p{...} classes with complements and
  # ranges, and every quantifier shape incl. possessive and counted.
  FixturePatterns = [
  ...
  # Hand-recorded text rows; expected segmentation comes from the
  # independent PCRE2 oracle, the explicit-expectation blocks below
...
proc main() =
  ...
  #
  # double-compile determinism: program bytes must hash identically
  #
  var detFails = 0
  ...
  #
  # NOTEMPTY: a zero-width acceptance is discarded, the scan moves on
  #
  block:
    var neFails = 0
  ...
  #
  # the $ assertion: end of subject or before a final LF (PCRE2
  # default newline mode), never strictly inside the subject
  #
  block:
    var dollarFails = 0
  ...
  # boundary, window-bounded dollar semantics
  #
  block:
    var anchoredFails = 0
  ...
  # table, dropped from White_Space in Unicode 6.3)
  #
  block:
    var wsFails = 0
    ...
    for cp in [0x180E'u32, 0x00A0'u32]:
      for p in [pcre, rust]:
        # bitmap predicate and range scan must agree on both probes
  ...
  #
  # oracle parity: engine segmentation == PCRE2 replica over every
  ...
  #
  # engine twin: frontier-DFA scan == NFA-simulation scan, byte
  # identical segmentation per fixture pattern
  #
  var twinFails = 0
  var twinCases = 0
  for i in 0 ..< compiled.len:
