...
##
## Probes the bundled PCRE2 engine (workspace/pcre2) for every codepoint
## of the Unicode range and emits sorted inclusive codepoint ranges for
## the character classes the engine's patterns use, plus the caseless-fold
## partner sets of the contraction letters. Probing the reference engine
## guarantees the tables carry the exact classes PCRE2 resolves, instead
## of a separately-sourced UCD snapshot that could drift.
##
## Run from the repo root (byte-determinism law: run twice, sha256 both
## outputs, commit only when the hashes match run-to-run):
##   nim cpp -r --outdir:build/tests/gen_unicode_tables \
##     --nimcache:nimcache/tests/gen_unicode_tables \
##     workspace/regex_engine/tests/testgen/gen_unicode_tables.nim
##
## Output (overwritten in place):
##   workspace/regex_engine/src/regex_unicode_tables.nim
...
const
  ...
  # exactly these letters. One capture per fold class: two distinct
  # ASCII letters never share a simple-fold class, so the alternation
  # never shadows.
...
proc main() =
  ...
  var outp =
    ...
    "## Generated by workspace/regex_engine/tests/testgen/" &
    "gen_unicode_tables.nim which probes the\n" &
    "## bundled PCRE2 engine for every codepoint, so the ranges match the\n" &
    "## exact UCD PCRE2 resolves. Do not hand-edit, do not commit by hand\n" &
    "## without rerunning the generator (run twice, both outputs must be\n" &
    "## byte-identical).\n" &
    "## Probe scope: codepoints 0x0000..0x10FFFF minus UTF-16 surrogates,\n" &
    "## " & $nProbed & " codepoints probed per pattern.\n\n"
