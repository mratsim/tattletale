...
import std/macros
import workspace/zstd/ast_rebuilder
...
macro replacePragmasByInline(procAst: typed): untyped =
  ...
  # keep the generic params of the wrapped proc
  result[^1][2] = procAst[2]
...
macro wrapOpenArrayLenType*(ty: typedesc, procAst: untyped): untyped =
  ...
  for i in 1 ..< procAst.params.len:
    if procAst.params[i][^2].kind == nnkBracketExpr and procAst.params[i][^2][0].eqident"openarray":
      ...
      # the wrappee fixes the element type at byte, char and byte
      # share the memory layout; the wrapper stays generic and casts
      wrappeeParams.add newIdentDefs(
        ident($procAst.params[i][0] & "Ptr"),
        nnkPtrTy.newTree(ident"byte"),
        newEmptyNode()
      )
      wrappeeParams.add newIdentDefs(
  ...
  # generic buffer elements (char|byte over the same C ABI) live on
  # the wrapper only, the wrappee stays non-generic so its importc
  # pragma expands in this module's scope
  wrapper[2] = procAst[2]
