...
when isMainModule:
  ...
  proc pack_and_test[M, N, K: static int; T](
    ...
    var packA = newSeq[T](packedA_size * 2)
    let rawA = cast[int](addr packA[0])
    let alignedA = (rawA + 63) and not 63
    let alignedAPtr = cast[ptr T](alignedA)

    var packB = newSeq[T](gemm_prepackB_mem_required(T, M, N, K) * 2)
    let rawB = cast[int](addr packB[0])
    let alignedB = (rawB + 63) and not 63
    let alignedBPtr = cast[ptr T](alignedB)
    ...
    gemm_packed(
      M, N, K,
      T(1), alignedAPtr, alignedBPtr,
      T(0), addr res_ab[0][0], N, 1
    )
  ...
  block:
    ...
    let b = [[1.0, 2, 3], [4.0, 5, 6], [7.0, 8, 9]]

    let ab = [[30.0, 36, 42], [66.0, 81, 96], [102.0, 126, 150]]
