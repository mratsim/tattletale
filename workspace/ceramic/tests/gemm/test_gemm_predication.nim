...
## bFragmentTileCol, cStoreMask) against an independent reference.
##
...
proc main() =
  # every thread: the A/B fragment rows/cols and the C store mask must
  # match the partition-view reference for the full and ragged extents
  for tid in 0 ..< blockSize:
    ...
    for i in 0 ..< size(tAv.layout):
      let refRow = (originA + toIntVal(crd2idx(tAv.layout, i))) mod TILE_M
      let got = aFragmentTileRow(tiled, thr, TILE_M, i)
      doAssert got == refRow,
        "aFragmentTileRow: thread " & $tid & " element " & $i & ": got " & $got &
        ", reference " & $refRow

    for i in 0 ..< size(tBv.layout):
      let refCol = (originB + toIntVal(crd2idx(tBv.layout, i))) mod TILE_N
      let got = bFragmentTileCol(tiled, thr, TILE_N, i)
      doAssert got == refCol,
        "bFragmentTileCol: thread " & $tid & " element " & $i & ": got " & $got &
        ", reference " & $refCol
  ...
  echo "  [OK] predication helpers: A/B fragment rows/cols and the C store" &
