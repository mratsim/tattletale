...
##
## References:
##   [CUTE-PAR] CuTe C++: atom/mma_atom.hpp — ThrMMA::partition_A/B/C,
##              thrfrg_A/B/C (constexpr layout algebra)
...
proc verifyFragments*[A, TL](mma: TiledMma[A, TL];
  ...
  ## expected.
  let
    tCount = mma.atom.threadCount(operand).toIntVal()
    vCount = mma.atom.fragmentValsPerThread(operand).toIntVal()
    tileSize = tileShape[0] * tileShape[1]
    thrM = mma.threadLayout.shape[0].toIntVal()
    thrN = mma.threadLayout.shape[1].toIntVal()
    thrK = mma.threadLayout.shape[2].toIntVal()
    expected = case operand
               of opA: thrN
               of opB: thrM
               of opC: thrK
    restCount = case operand
                of opA: (tileShape[0] div (thrM * mma.atom.mnk.m)) *
                        (tileShape[1] div (thrK * mma.atom.mnk.k))
                of opB: (tileShape[0] div (thrN * mma.atom.mnk.n)) *
                        (tileShape[1] div (thrK * mma.atom.mnk.k))
                of opC: (tileShape[0] div (thrM * mma.atom.mnk.m)) *
                        (tileShape[1] div (thrN * mma.atom.mnk.n))
  var counts = newSeq[int](tileSize)
  for thread in 0 ..< tCount * thrM * thrN * thrK:
    ...
    doAssert frag.len == vCount * restCount,
      &"per-thread fragment count (thread {thread})"
    for el in frag:
...
proc coords(frag: Fragment): seq[(int, int)] =
  ## The (row, col) coordinates of a fragment, in fragment order.
  for el in frag: result.add (el.row, el.col)
...
proc runDerivedQuantityTests =
  block:  # tile sizes derive from atom shape × thread tiling (3×5 tiled)
    ...
    check atom.fragmentValsPerThread(opA), 4, Int
    check atom.fragmentValsPerThread(opB), 2, Int
    check atom.fragmentValsPerThread(opC), 4, Int
  ...
  block:  # fragment size = V × rest per thread (3×5 tiled, rest (7,9,1))
    ...
    const mma = tiled(3, 5, 1)
    doAssert mma.partitionA((336, 8), 1).len == 28, "A fragment size"
    doAssert mma.partitionB((360, 8), 1).len == 18, "B fragment size"
    doAssert mma.partitionC((336, 360), 1).len == 252, "C fragment size"
    # fragment allocation == partition view (per-thread count is exact)
    doAssert mma.partitionA((336, 8), 1).len ==
      4 * (336 div 48) * (8 div 8)
    doAssert mma.partitionB((360, 8), 1).len ==
      2 * (360 div 40) * (8 div 8)
    doAssert mma.partitionC((336, 360), 1).len ==
      4 * (336 div 48) * (360 div 40)

  echo "  1. Derived quantities: 4 blocks OK"
...
proc runThreadTilingBoundaryTests =
  block:  # single atom (1×1), rest (1,1,1), thread 0
    ...
    doAssert coords(mma.partitionA((16, 8), 0)) == @[(0,0),(8,0),(0,4),(8,4)]
    doAssert coords(mma.partitionB((8, 8), 0)) == @[(0,0),(0,4)]
    doAssert coords(mma.partitionC((16, 8), 0)) == @[(0,0),(0,1),(8,0),(8,1)]
  ...
  block:  # 2×2 tiled, rest (1,1,1): threads 0 / 31 / 32 / 63 / 64 / 127
    ...
    for i, t in [0, 31, 32, 63, 64, 127]:
      doAssert coords(mma.partitionA((32, 8), t)) == expA[i], &"A t{t}"
      doAssert coords(mma.partitionB((16, 8), t)) == expB[i], &"B t{t}"
      doAssert coords(mma.partitionC((32, 16), t)) == expC[i], &"C t{t}"
...
proc runRestPositionTests =
  block:  # 1×1 tiled, rest (7,1)/(9,1)/(7,9) — pure value tiling, thread 0
    ...
    doAssert coords(mma.partitionA((112, 8), 0)) == expA,
      &"1×1 rest(7,1) A: {coords(mma.partitionA((112,8),0))}"
    let expB = @[
      (0,0),(0,4), (8,0),(8,4), (16,0),(16,4), (24,0),(24,4),
      (32,0),(32,4), (40,0),(40,4), (48,0),(48,4), (56,0),(56,4),
      (64,0),(64,4)]
    doAssert coords(mma.partitionB((72, 8), 0)) == expB, "1×1 rest(9,1) B"
    # C rest (7,9): size 252; corner samples at the (rn, rm) corners
    let fC = mma.partitionC((112, 72), 0)
    doAssert fC.len == 252
    doAssert coords(fC)[0..3] == @[(0,0),(0,1),(8,0),(8,1)]
    doAssert coords(fC)[4..7] == @[(16,0),(16,1),(24,0),(24,1)]
    doAssert coords(fC)[28..31] == @[(0,8),(0,9),(8,8),(8,9)]
    doAssert coords(fC)[248..251] == @[(96,64),(96,65),(104,64),(104,65)]
  ...
  block:  # 3×5 tiled, rest (7,9,1): first (t1) and last (t479) thread
    ...
    doAssert coords(mma.partitionA((336, 8), 1)) == expA1, "3×5 A t1"
    let expB1 = @[
      (0,1),(0,5), (40,1),(40,5), (80,1),(80,5), (120,1),(120,5),
      (160,1),(160,5), (200,1),(200,5), (240,1),(240,5), (280,1),(280,5),
      (320,1),(320,5)]
    doAssert coords(mma.partitionB((360, 8), 1)) == expB1, "3×5 B t1"
    let fC1 = mma.partitionC((336, 360), 1)
    doAssert fC1.len == 252
    doAssert coords(fC1)[0..3] == @[(0,2),(0,3),(8,2),(8,3)]
    doAssert coords(fC1)[248..251] == @[(288,322),(288,323),(296,322),(296,323)]
    ...
    doAssert coords(mma.partitionA((336, 8), 479)) == expA479, "3×5 A t479"
    doAssert coords(mma.partitionB((360, 8), 479))[0..3] == @[(39,3),(39,7),(79,3),(79,7)]
    doAssert coords(mma.partitionC((336, 360), 479))[0..3] == @[(39,38),(39,39),(47,38),(47,39)]
...
proc runKTiledTests =
  block:  # 2×2×2 tiled, rest (1,1,1): threads 0 / 128 (tk=1) / 255
    const mma = tiled(2, 2, 2)
    doAssert coords(mma.partitionA((32, 16), 0)) == @[(0,0),(8,0),(0,4),(8,4)]
    doAssert coords(mma.partitionB((16, 16), 0)) == @[(0,0),(0,4)]
    doAssert coords(mma.partitionC((32, 16), 0)) == @[(0,0),(0,1),(8,0),(8,1)]
    # thread 128: tk = 1 → k coords 8, 12; C unchanged (C has no K)
    doAssert coords(mma.partitionA((32, 16), 128)) == @[(0,8),(8,8),(0,12),(8,12)]
    doAssert coords(mma.partitionB((16, 16), 128)) == @[(0,8),(0,12)]
    doAssert coords(mma.partitionC((32, 16), 128)) == @[(0,0),(0,1),(8,0),(8,1)]
    # thread 255: atom (1,1,1), local thread 31
    doAssert coords(mma.partitionA((32, 16), 255)) == @[(23,11),(31,11),(23,15),(31,15)]
    doAssert coords(mma.partitionB((16, 16), 255)) == @[(15,11),(15,15)]
    doAssert coords(mma.partitionC((32, 16), 255)) == @[(23,14),(23,15),(31,14),(31,15)]
...
proc runRejectionAndInvariantTests =
  block:  # non-multiple tile shape is rejected (doAssert fires)
    const mma = tiled(2, 2, 1)
    try:
      discard mma.partitionA((33, 8), 0)   # 33 not a multiple of 32
  ...
  block:  # every thread of a 3×5 tiling has the same fragment SIZE
    const mma = tiled(3, 5, 1)
    for t in 0 ..< 480:
      doAssert mma.partitionA((336, 8), t).len == 28, &"A size t{t}"
      doAssert mma.partitionB((360, 8), t).len == 18, &"B size t{t}"
      doAssert mma.partitionC((336, 360), t).len == 252, &"C size t{t}"
