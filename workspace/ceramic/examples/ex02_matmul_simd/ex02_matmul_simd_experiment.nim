## ex02_matmul_simd — SIMD-accelerated serial GEMM — Experiment harness
##
## Compile-time toggles (each deactivates one optimization from the "fast" variant):
##   -d:expNoAlign         Use unaligned newSeq (like ref) instead of 32-byte manual alignment
##   -d:expNoRawEpilogue   Use TensorView epilogue (like ref) instead of raw-pointer epilogue
##   -d:expNoExplicitPackA Use fillWith_cpu(0) + copySameShape_cpu (like ref) instead of inline loops
##   -d:expNoExplicitPackB Use copySameShape_cpu (like ref) instead of inline loops for B pack
##   -d:expNoExplicitInner Use .slice() + displace (like ref) in inner loop instead of raw ptr
##
...
when defined(expNoRawEpilogue):
  # TensorView-based epilogue (like ref)
...
# ═══════════════════════════════════════════════════════════════════════════
#  gemm_strided — BLIS 5-loop GEMM (experiment harness)
# ═══════════════════════════════════════════════════════════════════════════
...
proc gemm_strided*[T: SomeNumber](
  ...
  when defined(expNoAlign):
    # Unaligned allocation (like ref)
    let packSizeA = int(cosize(packALay))
    let packSizeB = int(cosize(packBLay))
    var packDataA = newSeq[T](packSizeA)
    var packDataB = newSeq[T](packSizeB)
  ...
  var packA_ptr = cast[ptr UncheckedArray[T]](packA.data)
  var packB_ptr = cast[ptr UncheckedArray[T]](packB.data)
  ...
  for pc in 0 ..< num_pc:
    ...
    for jc in 0 ..< 1:
      ...
      let pB_rs = panelB.layout.stride[0]
      let pB_cs = panelB.layout.stride[1]

      when defined(expNoExplicitPackB):
        # Layout-algebra B pack (like ref)
        ...
        # Explicit B pack (fast)
        if pB_cs == 1:
          for jr in 0 ..< num_jr:
            for k in 0 ..< current_kc:
              let dstOff = jr * kc * nr + k * nr
              let srcOff = k * pB_rs + jr * nr
              copyMem(addr packB_ptr[dstOff], addr pB_ptr[srcOff], nr * sizeof(T).int)
        else:
          for jr in 0 ..< num_jr:
            for k in 0 ..< current_kc:
              let dstOff = jr * kc * nr + k * nr
              ...
              for jj in 0 ..< nr:
                packB_ptr[dstOff + jj] = pB_ptr[srcOff + jj * pB_cs]
      ...
      for ic in 0 ..< num_ic:
        ...
        when defined(expNoExplicitPackA):
          # Layout-algebra A pack with fillWith_cpu (like ref)
          if last_m or last_k:
        ...
        for jr in 0 ..< num_jr:
          ...
          when defined(expNoExplicitInner):
            let bTile = packB.slice((jr, _, _))
          else:
            let bOffset = jr * kc * nr
            ...
            when defined(expNoExplicitInner):
              ...
              when defined(expNoRawEpilogue):
                ...
                gemm_epilogue(activation, cast[ptr UncheckedArray[T]](cTile.data),
                              AB, mr, nr, alpha, beta, rowStrideC, colStrideC)
            else:
              gemm_ukernel(
                cast[ptr UncheckedArray[T]](cast[int](packA_ptr) +% aOffset *% sizeof(T).int),
                cast[ptr UncheckedArray[T]](cast[int](packB_ptr) +% bOffset *% sizeof(T).int),
                AB, current_kc)
              ...
              when defined(expNoRawEpilogue):
                ...
                let cPtr = cast[ptr UncheckedArray[T]](cast[int](C) +%
                  (cRow * rowStrideC + cCol * colStrideC) *% sizeof(T).int)
                gemm_epilogue(activation, cPtr,
                              AB, mr, nr, alpha, beta, rowStrideC, colStrideC)
