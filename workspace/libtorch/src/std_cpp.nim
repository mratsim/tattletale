...
proc `=destroy`[T](dst: CppSharedPtr[T]) {.importcpp: "#.~'*1()".}
  ## Skip Nim destructor and use C++ destructor
...
proc `=destroy`[T](dst: CppUniquePtr[T]) {.importcpp: "#.~'*1()".}
  ## Skip Nim destructor and use C++ destructor
...
type
  CppVector*[T]{.importcpp"std::vector", bycopy.} = object

proc init*(V: type CppVector): V {.importcpp: "std::vector<'*0>()", constructor.}
proc init*(V: type CppVector, size: int): V {.importcpp: "std::vector<'*0>(#)", constructor.}
proc len*(v: CppVector): int {.importcpp: "#.size()".}
proc add*[T](v: var CppVector[T], elem: T){.importcpp: "#.push_back(#)".}
proc `[]`*[T](v: CppVector[T], idx: int): T{.importcpp: "#[#]".}
proc `[]`*[T](v: var CppVector[T], idx: int): var T{.importcpp: "#[#]".}
