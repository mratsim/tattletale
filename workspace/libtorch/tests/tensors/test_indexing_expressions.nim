...
## Tests that indexing expressions evaluate the operand exactly once.
##
## Uses a `launchMissile` counter to verify the tensor expression is only
## evaluated once, even when the indexing macro accesses `t.shape` multiple times.
...
proc genShiftedVandermonde5x5*(dtype: ScalarKind): Tensor =
  (arange(1, 6, kFloat64).reshape(-1, 1) ** arange(1, 6).to(kFloat64)).to(dtype)
...
proc main() =
  ...
  runCppTest formatName("Full span shorthand _.._", "launchMissile(t)[_.._, _]"):
    proc(): bool =
      ...
      doAssert i == 1
      doAssert sliced.equal(vandermonde)
      true
  ...
  runCppTest formatName("Stepped span with index", "launchMissile(t)[|2, 0]"):
    proc(): bool =
      ...
      doAssert i == 1
      doAssert sliced.shape.len == 1
      doAssert sliced.equal(toTensor([1, 3, 5]).to(kFloat64))
      true
