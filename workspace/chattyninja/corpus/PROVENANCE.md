...
Extracted 2026-09-17 read-only from `tattletale@obsolete-20260910-chattyninja:workspace/chattyninja/tests/fixtures/`
via `git show` (repo main checkout, no worktree, no write-back). 243 blob files total:

- 18 `*.jinja` templates, 106 `*.json.zst` input frames, 106 `.meta.json` sidecars
- 6 `dsv4/test_input_*.json`, 5 `dsv4/test_output_*.txt`, `dsv4/rows.json`, `mistral7bv01/generated.json`
...
no tokenizer or model access. The POC supplies them as template context variables.
...
Ground truth origin is HF transformers `render_jinja_template`. Nothing was regenerated in this stage,
so no command was executed. If regeneration becomes necessary, the workspace-locked stack is
Python 3.14 with transformers 5.16.1, run from the workspace root:

```bash
cd /Users/pi/Documents/Programming/workspace-tattletale && uv run python <recorder>.py
```
...
- `dsv4/` carries no `.jinja`. Its 5 `test_input_*.json` / `test_output_*.txt` pairs are external fixtures,
  ...
  renderer tree, Apache-2.0, plain-text outputs (`rows.json` records this). Out of scope for the jinja engine POC
...
- `fixture_loader.nim` is the v1 Nim loader, a zstd plus JSON bridge with insertion-order-preserving
  ...
  compiled here, because the POC ships its own loader
