...
proc testNkIfBodyWalk(t: CompiledTemplate) =
  for i, n in t.nodes:
    if n.kind != nkIf:
      continue
    var cur = n.child
    var steps = 0
    while cur != n.succ:
      doAssert cur != NoLink and cur >= 0'i32 and cur < t.nodes.len.int32,
          "nkIf " & $i & " body does not terminate past the chain"
      doAssert cur != int32 i, "nkIf " & $i & " is re-entered by its own body path"
      doAssert steps <= t.nodes.len, "nkIf " & $i & " body walks a cycle"
      inc steps
...
proc testCommentCloseQuoteBlind() =
...
proc testTruncationRaisesLocated() =
...
proc main() =
  ...
  testNestingCapRaisesLocated()
  testElifChainNestingCap()
  testTruncationRaisesLocated()
  testLoneEndmacroGapRaises()
  testSetWithoutTargetNameRaises()
  testWhitespaceRuleSweep()

  echo "t_parse: ", tmpl.nodes.len, " nodes, ", symbols.names.len, " interned names, verbatim spans final"
